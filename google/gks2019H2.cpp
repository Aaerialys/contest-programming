...
int q,n,ans,t;
bool f[N][N],row[N*2],col[N*2];//row=x-y, col=x+y;
char ch;
...
int main(){
    ...
    fork(0,q){
        ...
        t=0;  seto(row,0); seto(col,0);
        fori(0,n) if(f[i][i]^row[N]) col[i*2]=true;
        fori(1,n) if(f[i][i-1]^row[N+1]) col[i*2-1]=true;
        fori(0,n) forj(0,n) if(f[i][j]^col[i+j]) row[i-j+N]=true;
        fori(0,N*2) t+=col[i];
        fori(0,N*2) t+=row[i];
        ans=min(ans,t);
        t=0;  seto(row,0); seto(col,0); row[N]=true;
        fori(0,n) if(f[i][i]^row[N]) col[i*2]=true;
        fori(1,n) if(f[i][i-1]^row[N+1]) col[i*2-1]=true;
        fori(0,n) forj(0,n) if(f[i][j]^col[i+j]) row[i-j+N]=true;
        fori(0,N*2) t+=col[i];
        fori(0,N*2) t+=row[i];
        ans=min(ans,t);
        t=0;  seto(row,0); seto(col,0); row[N+1]=true;
        fori(0,n) if(f[i][i]^row[N]) col[i*2]=true;
        fori(1,n) if(f[i][i-1]^row[N+1]) col[i*2-1]=true;
        fori(0,n) forj(0,n) if(f[i][j]^col[i+j]) row[i-j+N]=true;
        fori(0,N*2) t+=col[i];
        fori(0,N*2) t+=row[i];
        ans=min(ans,t);
        t=0;  seto(row,0); seto(col,0); row[N]=row[N+1]=true;
        fori(0,n) if(f[i][i]^row[N]) col[i*2]=true;
        fori(1,n) if(f[i][i-1]^row[N+1]) col[i*2-1]=true;
        fori(0,n) forj(0,n) if(f[i][j]^col[i+j]) row[i-j+N]=true;
        fori(0,N*2) t+=col[i];
