...
int main(){
    ...
    cin>>T;
    foru(cnt,1,T+1){
        cin>>n; cout<<"Case #"<<cnt<<": "; pos=x=y=false; seto(ans,0);
        fori(0,n){
            cin>>a>>b; task[i*2]={{a,1},i}; task[i*2+1]={{b,-1},i};
        }
        sort(task,task+n*2);
        fori(0,n*2){
            if(task[i].pf.ps<0){
                if(ans[task[i].ps]) x=false;
                else y=false;
                continue;
            }
            if(!x) x=ans[task[i].ps]=true;
            else if(!y) y=true;
            else pos=true;
        }
        if(pos){
