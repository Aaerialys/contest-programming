...
int main(){
    ...
    cin>>n;
    fori(0,n){
        cin>>in[i].pf.pf>>in[i].pf.ps;
        in[i].ps=i;
    }
    sort(in,in+n);
    fori(0,n){
        x[i]=in[i].pf.pf; b=in[i].pf.ps; l[i]=x[i]-b; r[i]=x[i]+b; poi.pb(l[i]); poi.pb(r[i]); poi.pb(x[i]);
    }
    sort(all(poi)); a=0;
    ...
    fori(0,n) ans[in[i].ps]=upper_bound(x,x+n,r[i])-lower_bound(x,x+n,l[i]);
    fori(0,n) cout<<ans[i]<<" ";
    return 0;
}
