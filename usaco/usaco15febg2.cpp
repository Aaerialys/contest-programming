...
struct AC{
    ...
    void go(int x){
        fori(0,26) if(nxt[x][i]==-1){
            if(ac[x].to[i]) nxt[x][i]=ac[x].to[i];
            else if(ac[x].link==-1) nxt[x][i]=0;
            else{
                if(nxt[ac[x].link][i]==-1) go(ac[x].link);
                nxt[x][i]=nxt[ac[x].link][i];
            }
        }
    }
    void init(){
...
int main(){
    cin.tie(0)->sync_with_stdio(0); fIO("censor");
    cin>>s>>m; n=sz(s);
    fori(0,m){
        cin>>t; ac.add(t);
    }
    ...
    for(auto i:s){
        x=nxt[x][i-'a'];
        ans.eb(x,i);
        fori(0,ac.ac[x].wlen) ans.pop_back();
        x=ans.back().pf;
    }
    for(auto i:ans) if(i.ps) cout<<i.ps;
    return 0;
}
/**
