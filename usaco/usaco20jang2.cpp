...
const int N=5001,Q=100010,M=2000010;
ll n,q,num[N],l,r,x,t,dp[N][N];
vector<int> sum[M*2];
...
int main()
{
    ...
    fori(3,n+1)
        forj(i,n+1)
        {
            dp[j-i+1][j]=dp[j-i+1][j-1]+dp[j-i+2][j]-dp[j-i+2][j-1];
            if(abs(num[j]+num[j-i+1])<M)
                dp[j-i+1][j]+=upper_bound(sum[-num[j-i+1]-num[j]+M].begin(),sum[-num[j-i+1]-num[j]+M].end(),j-1)-upper_bound(sum[-num[j-i+1]-num[j]+M].begin(),sum[-num[j-i+1]-num[j]+M].end(),j-i+1);
        }
    fori(0,q)
    {
        cin>>l>>r;
