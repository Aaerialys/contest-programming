...
const int N=2010,M=1e9+7,inv=500000004;
bool cow[N][N]; //stupid woolly cows
char ch;
ll n,dp[N][N][2],ans;
...
int main(){
    ...
    dp[0][1][1]=dp[1][0][0]=1; cow[0][1]=cow[1][0]=1;
    fori(1,n+2) forj(1,n+2){
        dp[i][j][0]=dp[i][j-1][0]; dp[i][j][1]=dp[i-1][j][1];
        if(!cow[i-1][j]) dp[i][j][0]+=dp[i-1][j][1]*inv%M;
        if(!cow[i][j-1]) dp[i][j][1]+=dp[i][j-1][0]*inv%M;
        dp[i][j][0]%=M; dp[i][j][1]%=M;
    }
    ans=dp[n+1][n][0]+dp[n][n+1][1];
    fori(1,n+1) forj(1,n+1) if(!cow[i][j]) ans=ans*2%M;
    cout<<ans;
    return 0;
}
/**
dp[i][j][k], k: 0=leading edge, 1=falling edge
dp[i][j][0]=dp[i][j-1]
if(!cow[i-1][j]) dp[i][j][0]+=dp[i-1][j][1]/2
dp[i][j][1]=dp[i-1][j][1]
if(cow[i][j-1]) dp[i][j][1]+=dp[i][j-1][0]/2
*/