...
const ll N=100010,M=P2(61)-1,B=10;//randll(100,M);
...
int n,m,S,a,b,c,arr[N],rnk[N];
ll hl[100],hsh[N],p[N];
gp_hash_table<ll,null_type> ts;
vector<int> rs,dumb;
void comb(int a,int b,ll t){
    t=(t+mul(hl[rs[a]],b))%M;
    if(a==sz(rs)-1){
        ts.insert(t);
        return;
    }
    fori(b+1,S+1) comb(a+1,i,t);
}
ll hshh(int l,int r){
    return (hsh[r]-mul(hsh[l-1],p[r-l+1])+M)%M;
}

int main(){
    cin.tie(0)->sync_with_stdio(0);
    p[0]=1;
    fori(1,N) p[i]=mul(p[i-1],B);
    cin>>n>>m>>S;
    fori(1,n+1) cin>>arr[i];
    fori(1,m+1) cin>>rnk[i];
    fori(1,m+1) hl[rnk[i]]=(hl[rnk[i]]+p[m-i])%M;
    rs.pb(0);
    fori(1,S+1) if(hl[i]) rs.pb(i);
    comb(0,0,0);
    dbg(ts);
    fori(1,n+1) hsh[i]=(mul(hsh[i-1],B)+arr[i])%M;
    fori(m,n+1) if(ts.find(hshh(i-m+1,i))!=ts.end()) dumb.pb(i-m+1);
    cout<<sz(dumb)<<nl;
    for(auto i:dumb) cout<<i<<nl;
    return 0;
}
/**
