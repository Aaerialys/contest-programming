...
const int N=255,M=MOD;
int n,m,a,b,c,d,e,s,t,lvl[N],nxt[N],mif,ob[N][4];
struct edge{
    int to,ind,flow,cap;
};
vector<edge> gr[N];
...
int main(){
    ...
    cin>>n;
    fori(1,n+1) forj(0,4) cin>>ob[i][j];
    fori(1,n+1) forj(0,4){
        if(ob[i][0]>ob[i][2]) swap(ob[i][0],ob[i][2]);
        else if(ob[i][1]>ob[i][3]) swap(ob[i][1],ob[i][3]);
    }
    fori(1,n+1) if(ob[i][0]==ob[i][2]) forj(1,n+1) if(i!=j&&ob[i][1]<=ob[j][1]&&ob[j][3]<=ob[i][3]&&ob[j][0]<=ob[i][0]&&ob[i][2]<=ob[j][2]) addEdge(i,j,1);
    fori(1,n+1){
        if(ob[i][0]==ob[i][2]) addEdge(0,i,1);
