...
const int N=1000010,R=1000,M=1e9+7;
ll parent[N],depth[N],n,m,head[N],preh[N],dp[N],t=1;
class djSet
{
public:
    djSet()
...
int main()
{
    ...
    ford(i,n-2,1)
    {
        ...
        forj(1,m)
            if(f[i*R+j]&&f[i*R+j+R])
                head[preh[i*R+j+R]]=lead(i*R+j);
        forj(1,m)
            if(f[i*R+j])
                preh[i*R+j]=lead(i*R+j);
    }
    fori(0,n)
        forj(0,m)
            if(head[i*R+j]!=i*R+j)
                gr[head[i*R+j]].pb(i*R+j);
    fori(0,n)
        forj(0,m)
            if(f[i*R+j]&&head[i*R+j]==i*R+j&&lead(i*R+j)==i*R+j)
            {
                dfs(i*R+j);
                t=t*dp[i*R+j]%M;
            }
