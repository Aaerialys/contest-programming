...
ll n,m,K,x,y,z,o[N],ans;
vector<ll> num[N];
minpq<tuple<ll,int,int>> nxt;
...
int main(){
    ...
    fori(0,n){
        ...
        sort(all(num[i])); y+=num[i][0];
    }
    iota(o,o+n,0); sort(o,o+n,[&](int a,int b){return num[a][0]-num[a][1]>num[b][0]-num[b][1];});
    nxt.emplace(y,0,0);
    fori(0,K){
        tie(x,y,z)=nxt.top(); nxt.pop();
        ans+=x;
        nxt.emplace(x+num[o[y]][z+1]-num[o[y]][z],y,z+1);
        if(y<n-1&&z>0) nxt.emplace(x+num[o[y+1]][1]-num[o[y+1]][0],y+1,1);
        if(y<n-1&&z==1) nxt.emplace(x+num[o[y]][0]-num[o[y]][1]+num[o[y+1]][1]-num[o[y+1]][0],y+1,1);
    }
    cout<<ans;
    return 0;
}
