...
const int N=1000010;
int n,x;
double pos[N],p=1,q,ans;

int main()
{
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("3.in", "r", stdin); //freopen("", "w", stdout);
    cin>>n;
    fori(0,n)
    {
        cin>>x;
    ...
    fori(0,n)
    {
        p*=pos[i]; q+=(1-pos[i])/pos[i];
        for(;x<i&&p/pos[x]*(q-(1-pos[x])/pos[x])>p*q;x++)
        {
            p/=pos[x];
            q-=(1-pos[x])/pos[x];
        }
...
/** you can prove that the best window is smallest
