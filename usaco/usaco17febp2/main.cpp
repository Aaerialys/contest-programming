...
template<class T1,class T2>constexpr bool ckmax(T1&x,const T2&y){return x<y?x=y,1:0;} template<class T,class...Ts>constexpr bool ckmax(T&x,const Ts&...xs){return ckmax(x,min(xs...));}
struct chash{
...
int main(){
    cin.tie(0)->sync_with_stdio(0); fIO("nocross");
    cin>>n;
    fori(0,n) cin>>a[i];
    fori(0,n) cin>>x,b[x]=i;
    fori(0,n){
        vector<int> temp;
        ford(j,min(n,a[i]+4),max(1,a[i]-4)) temp.pb(b[j]);
        sort(all(temp),greater<int>());
        for(auto j:temp)
            bit.upd(j,bit.query(j-1)+1);
    }
    cout<<bit.query(N-2)<<nl;
    return 0;
}
/**
0 0 0 0 0 0
