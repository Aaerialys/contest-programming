...
int main()
{
    ...
    {
        cin>>a>>b>>c>>d;
        sp.pb({a,b,1,i}); sp.pb({c,d,0,i});
        poi.pb(b); poi.pb(d);
    }
    sort(sp.begin(),sp.end(),cmp); sort(poi.begin(),poi.end()); reverse(poi.begin(),poi.end());
    fori(0,poi.size())
        cord[poi[i]]=i+1;
    bit.update(cord[n],n*2);
    for(auto i:sp)
    {
        if(i.t)
            bit.update(cord[i.y],dp[i.i]+i.x+i.y);
        else
            dp[i.i]=bit.query(cord[i.y])-i.y-i.x;
    }
    cout<<bit.query(poi.size());
    return 0;
}
/**
