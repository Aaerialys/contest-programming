...
int n,m,rt,a,b,low[N],dfn[N],inx,scc,id[N],sz[N],ans;
bool ins[N],vis[N];
stack<int> curg;
vector<int> gr[N],ord;
void tarjam(int v){
...
void dfs(int v){
    if(v==id[v]) ord.pb(v);
    sz[id[v]]++; vis[v]=1;
    for(auto i:gr[v]) if(!vis[i]){
        if(id[i]!=id[v]&&!vis[id[i]]){
            dfs(i);
            sz[id[v]]+=sz[id[i]];
        }
        else dfs(i);
