...
const int N=3010,MN=2000010,M=1e9+7;
ll H,W,n,fact[MN],way[N];
pii wall[N];
int c(int a,int b){
    return fact[a]*fpow(fact[a-b]*fact[b]%M,M-2)%M;
}
...
int main(){
    ...
    fori(0,n) cin>>wall[i].pf>>wall[i].ps;
    sort(wall,wall+n); wall[n++]={H,W};
    fori(0,n) way[i]=calc(wall[i].pf-1,wall[i].ps-1);
    fori(0,n) forj(i+1,n) if(wall[j].pf>-wall[i].pf&&wall[j].ps>=wall[i].ps){
        way[j]=(way[j]-way[i]*calc(wall[j].pf-wall[i].pf,wall[j].ps-wall[i].ps)%M+M)%M;
    }
    cout<<way[n-1];
    return 0;
}
