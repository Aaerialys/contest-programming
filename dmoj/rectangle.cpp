...
ll n,x[N],y[N],ans;
map<tuple<int,int,ll>,int> p;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>n;
    fori(0,n) cin>>x[i]>>y[i];
    fori(0,n) forj(0,i) p[make_tuple(x[i]+x[j],y[i]+y[j],(x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]))]++;
    for(auto i:p) ans+=i.ps*(i.ps-1)/2;
    cout<<ans;
    return 0;
}
/**
