...
int u,v,x1,x2,x3,x4,Y1,y2,y3,y4;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>x1>>Y1>>x2>>y2>>x3>>y3>>x4>>y4; u=(y4-y3)*(x2-x1)-(x4-x3)*(y2-Y1); v=(x4-x3)*(Y1-y3)-(y4-y3)*(x1-x3);
    if(!u){
        if(v) cout<<"parallel";
        else cout<<"coincident";
        return 0;
    }
    printf("%.8f %.8f",x1+1.0*v/u*(x2-x1),Y1+1.0*v/u*(y2-Y1));
    return 0;
}
/**
