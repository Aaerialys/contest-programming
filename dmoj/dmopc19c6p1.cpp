...
#define xc real()
#define yc imag()
complex<double> p[4],ans;
int a,b;
double c,d,EPS=1e-3;
double cross(complex<int> a,complex<int> b){
    return (conj(a)*b).yc;
}
int main(){
    ...
    if(p[0].xc>p[1].xc||abs(p[0].xc-p[1].xc)<EPS&&p[0].yc>p[1].yc) swap(p[0],p[1]);
    if(p[2].xc>p[3].xc||abs(p[2].xc-p[3].xc)<EPS&&p[2].yc>p[3].yc) swap(p[2],p[3]);
    if(p[0].xc>p[2].xc||abs(p[0].xc-p[2].xc)<EPS&&p[0].yc>p[2].yc){
        swap(p[0],p[2]);
        swap(p[1],p[3]);
    }
    if(abs(arg(p[0]-p[1])-arg(p[2]-p[3]))<EPS){
        if(abs(arg(p[0]-p[1])-arg(p[0]-p[3]))<EPS) cout<<"coincident";
        else cout<<"parallel";
        return 0;
    }
    c=cross(p[2]-p[0],p[1]-p[0]); d=cross(p[3]-p[0],p[1]-p[0]);
    ans=(c*p[3]-d*p[2])/(c-d);
    printf("%.6f %.6f",ans.xc,ans.yc);
    return 0;
}
/**
