...
int main()
{
    ...
    fori(0,N)
    {
        s=to_string(i); t=s; reverse(t.begin(),t.end());
        //cout<<s<<" "<<t<<"\n";
        pal.pb(stoi(s+t)); pal.pb(stoi(s.substr(0,s.size()-1)+t));
    }
    sort(pal.begin(),pal.end()); a=x;
    for(auto i:pal)
        if(i>=x&&i<y&&min(abs(i-x),abs(i-y))>=min(abs(a-x),abs(a-y)))
            a=i;
    cout<<a;
    return 0;
}
/**
(800+(800-400(x-1)))/2*x=1000
*/