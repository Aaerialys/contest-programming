...
const int N=100010;
ll n,m,K,a,b,c,dist[11][N],vis[10],ord[10];
vector<pii> gr[N];
...
int main()
{
    ...
    a=10; dist[10][0]=0; c=INF; toV.push({0,0});
    while(!toV.empty())
        dfs();
    do
    {
        b=dist[10][vis[ord[0]]]+dist[10][vis[ord[K-1]]];
        fori(1,K)
            b+=dist[ord[i]][vis[ord[i-1]]];
        c=min(c,b);
