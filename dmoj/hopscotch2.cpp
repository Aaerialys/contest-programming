...
const int N=10000000,M=1e9+7;
ll n,dp[N],K,x,last[N];
deque<int> nxt;
stack<int> out;
...
int main(){
    ...
    fori(1,n+1) cin>>dp[i];
    fori(1,n+2){
        while(sz(nxt)&&nxt.front()+K<i) nxt.pop_front();
        dp[i]+=dp[nxt.front()]; last[i]=nxt.front();
        while(sz(nxt)&&dp[i]<=dp[nxt.back()]) nxt.pop_back();
        nxt.pb(i);
    }
    x=last[n+1];
    while(x){
    ...
    cout<<dp[n+1]<<"\n";
    for(;sz(out);out.pop()) cout<<out.top()<<" ";
    return 0;
}
/**
