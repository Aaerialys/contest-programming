...
int main(){
    ...
    fori(1,n+1){
        cin>>ch;
        if(ch=='1') ans=(ans+fpow(2,n-i))%M;
        else zero.insert(i);
    }
    fori(0,m){
        cin>>l>>r;
        for(auto i=zero.lower_bound(l);i!=zero.upper_bound(r);i++) ans=(ans+fpow(2,n-*i))%M;
        zero.erase(zero.lower_bound(l),zero.upper_bound(r));
        cout<<ans<<"\n";
    }
    return 0;
}
