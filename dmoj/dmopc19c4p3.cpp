...
ll n,m,v1[N],v2[N],c1[N],c2[N],dp[N][101],t;

int main()
{
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>n>>m; seto(dp,-inf); dp[0][0]=0;
    fori(1,n+1)
        cin>>c1[i]>>c2[i]>>v1[i]>>v2[i];
    fori(1,n+1)
    {
        forj(0,M)
            fork(0,min(c2[i],100LL-j)+1)
                dp[i][j]=max(dp[i][j],dp[i-1][j+k]+v2[i]*k);
        ford(j,M-1,0)
            ford(k,(ll)min(0LL+j,c1[i]),0)
                dp[i][j]=max(dp[i][j],dp[i][j-k]-v1[i]*k);
        forj(0,M)
            dp[i][j]-=m*j;
    }
    cout<<dp[n][0];
