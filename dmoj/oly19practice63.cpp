...
const int N=2001;
ll n,m,nu[N],nv[N],dp[N][N],w[N],uv[N];

int main()
{
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>n>>m; seto(dp,inf); dp[n][m]=0; seto(w,inf); seto(uv,inf);
    fori(1,n+1)
    {
        cin>>nu[i];
        nu[i]+=nu[i-1]-1;
    ...
    ford(a,n,0)
        ...
        {
            if(a<n&&b<m)
                dp[a][b]=min(w[b]-nu[a]*(nv[b+1]-nv[b]),uv[a]-nv[b]*(nu[a+1]-nu[a]));
            w[b]=min(w[b],dp[a][b+1]+nu[a]*(nv[b+1]-nv[b]));
            uv[a]=min(uv[a],dp[a+1][b]+(nu[a+1]-nu[a])*nv[b]);
        }
    cout<<dp[0][0];
    return 0;
}
