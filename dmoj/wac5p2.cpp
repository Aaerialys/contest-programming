...
int a[N],mi[N],ma[N],n,q,x,y,aa,bb;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>n>>q; ma[n]=-inf; mi[n]=inf;
    fori(0,n) cin>>a[i];
    ford(i,n-1,0) mi[i]=min(mi[i+1],a[i]),ma[i]=max(ma[i+1],a[i]);
    reverse(ma,ma+n);
    while(q--){
        cin>>x>>y;
        aa=n-(lower_bound(mi,mi+n,x-y)-mi); bb=upper_bound(ma,ma+n,x+y)-ma;
        cout<<min(aa,bb)<<nl;
    }
    return 0;
}
