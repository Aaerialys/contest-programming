...
int n,m,a,b,dp[N],ml[N],mr[N];
deque<int> best;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>n>>m; seto(dp,-inf); seto(ml,inf);
    fori(0,m){
        cin>>a>>b; ml[b]=min(ml[b],a); mr[b+1]=max(mr[b+1],a);
    }
    fori(1,n+2) mr[i]=max(mr[i],mr[i-1]);
    ford(i,n+2,0) ml[i]=min(ml[i],ml[i+1],i);
    dp[0]=0; best.pb(0);
    fori(1,n+2){
        for(;sz(best)&&best.front()<mr[i];best.pop_front());
        if(sz(best)) dp[i]=max(dp[i],dp[best.front()]+1);
        forj(ml[i],ml[i+1]){
            for(;sz(best)&&dp[best.back()]<dp[j];best.pop_back());
            best.pb(j);
        }
    }
    cout<<max(-1,dp[n+1]-1);
