...
int n,a[N],x[N],y[N],x0,Y[N],t,ma,mi,xx;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>n; ma=inf;
    fori(0,n) cin>>a[i],t+=a[i];
    x[0]=1;
    fori(1,n){
        x[i]=-x[i-1];
        y[i]=a[i]-y[i-1];
        if(x[i]==-1) ma=min(ma,y[i]);
        if(x[i]==1) mi=max(mi,-y[i]);
    }
    x[0]=-x[n-1]; y[0]=a[0]-y[n-1];
    if(x[0]==1&&!y[0]) x0=mi;
    else if(x[0]==-1&&y[0]%2==0) x0=y[0]/2;
    else{
        cout<<"NO";
        return 0;
    }
