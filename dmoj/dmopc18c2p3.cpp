...
int n,m,K,a,b,x,y,v,mid[N],dist[N],disf[N],ans;
vector<int> gr[N];
queue<int> toV;
...
int main(){
    ...
    cin>>n>>m>>K>>x>>y; seto(dist,inf); seto(disf,inf); ans=inf;
    fori(0,K) cin>>mid[i];
    fori(0,m){
        cin>>a>>b;
        gr[a].pb(b); gr[b].pb(a);
    ...
    while(sz(toV)){
        v=toV.front(); toV.pop();
        for(auto i:gr[v]) if(dist[i]>dist[v]+1){
            dist[i]=dist[v]+1;
            toV.push(i);
        }
    }
    ...
    while(sz(toV)){
        v=toV.front(); toV.pop();
        for(auto i:gr[v]) if(disf[i]>disf[v]+1){
            disf[i]=disf[v]+1;
            toV.push(i);
        }
    }
    fori(0,K) ans=min(ans,dist[mid[i]]+disf[mid[i]]);
    cout<<ans;
    return 0;
}
/**
