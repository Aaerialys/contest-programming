...
int n,h[N],ans[N],a,b,x;
deque<int> dq;
double f(int i,int x){
    return h[x]+sqrt(abs(i-x));
}
int p(int x,int y){
    int ret=y;
    ford(i,20,0) if(f(ret+P2(i),x)>f(ret+P2(i),y)) ret+=P2(i);
    return ret;
}
int main(){
    ...
    dq.pb(0);
    fori(1,n){
        while(sz(dq)>=2&&p(dq[0],dq[1])<=i) dq.pop_front();
        ans[i]=max(ans[i],ceil(f(i,dq[0])));
        if(h[i]<=h[dq.back()]) continue;
        while(sz(dq)>=2&&p(dq[sz(dq)-2],dq[sz(dq)-1])>=p(dq[sz(dq)-1],i)) dq.pop_back();
        dq.pb(i);
    }
    reverse(h,h+n); reverse(ans,ans+n); dq.clear(); dq.pb(0);
    fori(1,n){
        while(sz(dq)>=2&&p(dq[0],dq[1])<=i) dq.pop_front();
        ans[i]=max(ans[i],ceil(f(i,dq[0])));
        if(h[i]<=h[dq.back()]) continue;
    ...
    fori(0,n) cout<<max(0,ans[i]-h[i])<<nl;
    return 0;
}
/**
