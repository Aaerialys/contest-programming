...
const int N=P2(16),M=MOD,L=16;
ll n,dp[N],v[N],e[L][L];

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>n;
    fori(0,n) forj(0,n) cin>>e[i][j];
    fori(0,P2(n)) forj(0,n) if(i&P2(j)) fork(0,j) if(i&P2(k)) v[i]+=e[j][k];
    fori(0,P2(n)){
        vector<int> cur;
        forj(0,n) if(i&P2(j)) cur.pb(P2(j));
        forj(0,P2(sz(cur)-1)){
            int x=0;
            fork(0,sz(cur)-1) if(j&P2(k)) x+=cur[k];
            dp[i]=max(dp[i],dp[x]+v[i^x]);
        }
    }
    cout<<dp[P2(n)-1];
    return 0;
