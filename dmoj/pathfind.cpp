...
int main(){
    ...
    fori(0,m) if(blo[1][i]) toV.emplace(1,i),blo[1][i]=0;
    fori(0,n) if(blo[i][m]) toV.emplace(i,m),blo[i][m]=0;
    while(sz(toV)){
        tie(x,y)=toV.front(); toV.pop();
        if(x==n||y==1){
            cout<<"NO";
            return 0;
        }
        fori(-1,2) forj(-1,2) if(blo[x+i][y+j]) toV.emplace(x+i,y+j),blo[x+i][y+j]=0;
    }
    cout<<"YES";
    return 0;
}
