...
const int N=100010;
ll M,n,ans[N],a,b,c,x[N];
ll fpow(ll a,ll b){if(!b) return 1; ll t=fpow(a,b/2); if(b&1) return t*t%M*a%M; return t*t%M;}
vector<ll> gr[N],dumb[N],dump[N];
ll dfs(int v,int p=0){
    x[v]=1; dumb[v].pb(1); dump[v].pb(1);
    for(auto i:gr[v]) if(i!=p) x[v]=x[v]*dfs(i,v)%M;
    for(auto i:gr[v]) if(i!=p) dumb[v].pb(dumb[v].back()*(x[i]+1)%M);
    reverse(all(gr[v]));
    for(auto i:gr[v]) if(i!=p) dump[v].pb(dump[v].back()*(x[i]+1)%M);
    reverse(all(gr[v])); reverse(all(dump[v]));
    return x[v]+1;
}
void dfs2(int v,int p=0){
    ans[v]=x[v]; int stupid=0;
    for(auto i:gr[v]) if(i!=p){
        a=x[i]; b=x[v]; x[v]=dumb[v][stupid]*dump[v][stupid+1]%M*(x[p]+1)%M; stupid++; x[i]=x[i]*(x[v]+1)%M;
        dfs2(i,v);
        x[i]=a; x[v]=b;
    }
}
