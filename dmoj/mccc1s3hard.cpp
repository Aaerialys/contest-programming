...
#define fori(a,b) for(int i=a,ENDI=b;i<ENDI;i++)
...
int main(){
    ...
    forj(0,q)
    {
        ...
        else
        {
            ...
            if(in.size()<=s.size()||in.substr(0,s.size())!=s)
            {
                cout<<"1\n";
                continue;
            }
            int tt=in.size();
            fori((int)s.size(),in.size()+1)
            {
                if(i==in.size()||!nxt[a][in[i]-'a'])
                {
                    cout<<a<<"\n";
                    break;
                }
