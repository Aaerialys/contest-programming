...
int main(){
    ...
    fori(0,n){
        ...
        dq[0].pb(0);
        forj(0,N){
            while(dq[j%w].size()&&dq[j%w].front()<j-u*w) dq[j%w].pop_front();
            while(dq[j%w].size()&&dp[i&1^1][dq[j%w].front()]+(j-dq[j%w].front())/w*v<dp[i&1^1][j]) dq[j%w].pop_back();
            dq[j%w].pb(j);
            dp[i&1][j]=dp[i&1^1][dq[j%w].front()]+(j-dq[j%w].front())/w*v;
        }
    }
    fori(0,m){
        cin>>u>>w;
        ans=max(ans,dp[n&1^1][u]-w);
    }
    cout<<ans;
    return 0;
}
