...
string s,t,ans;
int x,y,dp[N][N],pre[N][N],xx;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>s>>t; s="."+s; t="."+t;
    fori(1,sz(s)) forj(1,sz(t)){
        dp[i][j]=dp[i-1][j]; pre[i][j]=2;
        if(dp[i][j-1]>dp[i][j]) pre[i][j]=1,dp[i][j]=dp[i][j-1];
        if(s[i]==t[j]&&dp[i-1][j-1]+1>dp[i][j]) dp[i][j]=dp[i-1][j-1]+1,pre[i][j]=0;
    }
    x=sz(s)-1; y=sz(t)-1;
    while(x&&y){
        xx=pre[x][y];
        if(!xx) ans=string(1,s[x])+ans;
        x-=!(xx&1); y-=!(xx&2);
    }
    cout<<ans;
    return 0;
