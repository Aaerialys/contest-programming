...
const int N=40,P=P2(21),M=MOD;
ll n,m,A,v[N],sum[P],x,y,ans,p[N]={1},rp[P];
vector<ll> way1,way2;
...
int main(){
    ...
    fori(0,n) cin>>v[i];
    fori(0,p[A]){
        sum[i]=i? sum[i-(i&-i)]+v[rp[i&-i]]:0;
        way1.pb(sum[i]);
    }
    sort(all(way1),greater<ll>());
    seto(sum,0); way2.pb(INF);
    fori(0,p[n-A]){
        sum[i]=i? sum[i-(i&-i)]+v[rp[i&-i]+A]:0;
        way2.pb(sum[i]);
    }
    sort(all(way2));
    for(auto i:way1){
