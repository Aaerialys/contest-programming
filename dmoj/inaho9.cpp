...
string z;
ll n,a[N],ans[N][N],temp[N][N],ret[N][N],mat[N][N],x;
void m1(){
    seto(ret,0);
    fori(0,n) forj(0,n) fork(0,n){
        ret[i][j]=(ret[i][j]+ans[i][k]*temp[k][j])%M;
    }
    fori(0,n) forj(0,n) ans[i][j]=ret[i][j];
}
void m2(){
    seto(ret,0);
    fori(0,n) forj(0,n) fork(0,n){
        ret[i][j]=(ret[i][j]+ans[i][k]*mat[k][j])%M;
    }
    fori(0,n) forj(0,n) ans[i][j]=ret[i][j];
}
...
int main(){
    ...
    fori(0,n) cin>>a[i];
    fori(1,n) mat[i][i-1]=1;
    fori(0,n) mat[0][i]=a[i];
    fori(0,n) ans[i][i]=1;
    for(auto d:z){
        fori(0,n) forj(0,n) temp[i][j]=ans[i][j];
        fori(0,9) m1();
        fori(0,d-'0') m2();
    }
    fori(0,n) x+=ans[n-1][i];
    cout<<x%M;
    return 0;
}
/**
