...
ll n,a,b,c,dp[N],ans;
vector<array<int,3>> bloc;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>n; bloc.resize(n);
    fori(0,n) cin>>bloc[i][0]>>bloc[i][1]>>bloc[i][2];
    sort(all(bloc),[](array<int,3> a,array<int,3> b){return min(a[1],b[1]-a[0])<min(b[1],a[1]-b[0]);});
    seto(dp,-inf); ans=-inf;
    for(auto i:bloc){
        forj(i[0],N) dp[min(j-i[0],i[1])]=max(dp[min(j-i[0],i[1])],dp[j]+i[2]);
        dp[i[1]]=max(dp[i[1]],i[2]);
    }
    fori(0,N) ans=max(ans,dp[i]);
    cout<<ans;
    return 0;
}
/**
