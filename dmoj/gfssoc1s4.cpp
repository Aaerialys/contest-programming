...
const int N=256,M=1e9+7;
ull n,q,bit[N][N][N],x0,Y0,z0,x1,Y1,z1,ans;
char ch;
void update(int x,int y,int z,ull v){
...
ull query(int x0,int Y0,int z0,int x1,int Y1,int z1){
    x0--; Y0--; z0--;
    return query(x1,Y1,z1)-query(x0,Y1,z1)-query(x1,Y0,z1)-query(x1,Y1,z0)+query(x1,Y0,z0)+query(x0,Y1,z0)+query(x0,Y0,z1)-query(x0,Y0,z0);
}
...
int main(){
    ...
    fori(0,q){
        cin>>ch;
        if(ch=='C'){
            cin>>x0>>Y0>>z0>>x1; x1-=query(x0,Y0,z0,x0,Y0,z0);
            update(x0,Y0,z0,x1);
        }
        if(ch=='S'){
            cin>>x0>>Y0>>z0>>x1>>Y1>>z1;
            ans+=query(x0,Y0,z0,x1,Y1,z1);//cout<<query(x0,Y0,z0,x1,Y1,z1)<<"\n";
        }
    }
    cout<<ans;
    return 0;
