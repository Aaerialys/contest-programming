...
lld f(lld aa)
{
    return co[0]+aa*co[1]+aa*aa*co[2]+aa*aa*aa*co[3]+aa*aa*aa*aa*co[4]+aa*aa*aa*aa*aa*co[5];
}
bool cross(lld aa,lld bb)
{
...
int main()
{
    ...
    cin>>l>>r; a=f(l); x=1; inc=(r-l)/20000000.0;
    for(lld i=l;i<=r;i+=inc)
    {
        c=b; b=a; a=f(i+inc);
        if(cross(a,b))
        {
