...
ll ans;
vector<pii> gr[N];
unordered_map<ll,ll> dist,cdist;
char ch;
bool block[N];
...
void dfs(int v,int p,int d){
    cdist[d]++;
    for(auto i:gr[v]) if(i.pf!=p&&!block[i.pf]) dfs(i.pf,v,d+i.ps);
}
void go(int v){
    ...
    for(auto i:gr[v])
        if(!block[i.pf]){
            cdist.clear(); dfs(i.pf,v,i.ps);
            for(auto j:cdist)
                ans+=j.ps*(dist[-j.pf-1]+dist[-j.pf+1]);
            for(auto j:cdist)
                dist[j.pf]+=j.ps;
            ans--;
        }
...
int main(){
    ...
    fori(1,n){
        cin>>a>>b>>ch;
        gr[a].pb({b,ch=='r'? 1:-1}); gr[b].pb({a,ch=='r'? 1:-1});
    }
    go(1);
    cout<<ans;
    return 0;
