...
int n,a,b,c,d,x,y,z,p[L][N],dep[N],q;
vector<int> gr[N];
void dfs(int v,int par=0){
    dep[v]=dep[par]+1; p[0][v]=par;
    for(auto i:gr[v]) if(i!=par) dfs(i,v);
}
int lca(int u,int v){
    if(dep[u]>dep[v]) swap(u,v);
    ford(i,L-1,0) if(dep[u]<=dep[p[i][v]]) v=p[i][v];
    if(u==v) return u;
    ford(i,L-1,0) if(p[i][u]!=p[i][v]) u=p[i][u],v=p[i][v];
    return p[0][u];
}
...
int main(){
    ...
    while(q--){
        cin>>a>>b>>c>>d;
        x=dep[a]+dep[c]-2*dep[lca(a,c)];
        if(x&1){
            cout<<-1<<nl;
            continue;
        }
        x/=2; y=dep[a]>dep[c]? a: c; z=dep[y]-x;
        ford(i,L-1,0) if(dep[p[i][y]]>=z) y=p[i][y];
        if(y==b||y==d){
            cout<<-1<<nl;
            continue;
        }
        if((lca(a,y)==y||lca(b,y)==y)&&z>=dep[lca(a,b)]&&(lca(c,y)==y||lca(d,y)==y)&&z>=dep[lca(c,d)]) cout<<x<<nl;
        else cout<<-1<<nl;
    }
    return 0;
}
