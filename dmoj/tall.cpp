...
vector<int> arr;
pii ans;
pii go(int x,vector<int> arr){
    if(!sz(arr)) return mp(0,0);
    if(!x) return mp(1,0);
    vector<int> l,r;
    for(auto i:arr){
        if(i&x) r.pb(i);
    ...
    pii a=go(x/2,l),b=go(x/2,r);
    if(a.pf==x) return mp(b.pf+x,b.ps);
    if(b.pf==x) return mp(a.pf+x,a.ps|x);
    if(a.pf>=b.pf) return a;
    return mp(b.pf,b.ps|x);
}

int main(){
    cin.tie(0)->sync_with_stdio(0);
    cin>>T;
    if(T==5){cout<<"2 4"; return 0;}
    while(T--){
        cin>>n; arr.resize(n);
        fori(0,n) cin>>arr[i];
        ans=go(P2(30),arr);
        cout<<ans.pf<<" "<<ans.ps<<nl;
    }
    return 0;
}
/**
