...
ll n,K,dp[N][N],a,b,c,sz[N];
vector<pll> gr[N];
void dfs(int v,int p){
    sz[v]=1; dp[v][0]=dp[v][1]=0;
    for(auto i:gr[v]) if(i.pf!=p){
        dfs(i.pf,v);
        sz[v]+=sz[i.pf];
        ford(j,min(K,sz[v]),0) fork(0,min(j,sz[i.pf])+1) dp[v][j]=max(dp[v][j],dp[v][j-k]+dp[i.pf][k]+i.ps*k*(K-k)+i.ps*(sz[i.pf]-k)*(n-sz[i.pf]-K+k));
    }
}
