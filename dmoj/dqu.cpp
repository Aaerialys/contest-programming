...
int main(){
    ...
    cin>>n>>q; n=min(n,q+1);
    fori(0,q){
        cin>>a>>b>>c; b=min(b,q+1); c=min(c,q+1);
        if(a==2) que.eb(i,2,b,c);
        else upd.eb(b,i),upd.eb(c+1,i);
    }
    sort(all(upd));
    fori(1,n+1){
        for(;x<sz(upd)&&upd[x].pf==i;x++){
            if(inc.find(upd[x].ps)!=inc.end()) inc.erase(upd[x].ps);
            else inc.insert(upd[x].ps);
        }
        for(int j=i-1;j<sz(inc);j+=i) que.eb(*inc.find_by_order(j),1,i,i);
    }
    sort(all(que));
    for(auto i:que){
        tie(ignore,a,b,c)=i;
        if(a==1) bit.upd(b,1);
        else cout<<bit.query(c)-bit.query(b-1)<<nl;
    }
    return 0;
}
/**
