...
const ll N=1000010,M=P2(61)-1,B=randll(256,M-1);
...
ll n,hashnum[N],p[N]={1},K,a;
string in;
ll hsh(int l,int r){
    return (hashnum[r]-modmul(hashnum[l-1],p[r-l+1])+M)%M;
}
bool cmp(int l1,int r1,int l2,int r2){
    int l=0,r=min(r1-l1,r2-l2),mid;
    while(l<r){
        mid=(l+r)/2;
        if(hsh(l1,l1+mid)==hsh(l2,l2+mid))
            l=mid+1;
        else
            r=mid;
    }
    if(l==min(r1-l1,r2-l2))
        return r1-l1<r2-l2;
    return in[l1+l]<in[l2+l];
}

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    fori(1,N) p[i]=modmul(p[i-1],B);
    cin>>in>>K; in="."+in; n=in.size(); a=K;
    fori(1,n) hashnum[i]=modmul(hashnum[i-1],B)+in[i];
    fori(K,n)
        if(cmp(i-K+1,i,a-K+1,a)){
            a=i;
        }
    cout<<in.substr(a-K+1,K);
    return 0;
}
/**
