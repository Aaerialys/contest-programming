...
int main(){
    ...
    while(q--){
        ...
        if(ch=='A'){
            cin>>x;
            if(pos.find(x)!=pos.end()) cout<<"Can't add "<<x<<nl;
            else{
                bit.upd(cur,val(x));
                pos[x]=cur++;
            }
            ...
            cin>>x>>y;
            bit.upd(pos[x],val(y)-val(x)); bit.upd(pos[y],val(x)-val(y)); swap(pos[x],pos[y]);
        }
        else if(ch=='M'){
            cin>>x>>y;
            cout<<bit.query(max(pos[x],pos[y]))-bit.query(min(pos[x],pos[y])-1)<<nl;
        }
        else if(ch=='R'){
            cin>>x>>y; pos[y]=pos[x]; pos.erase(x);
            bit.upd(pos[y],val(y)-val(x));
        }
        else if(ch=='N') cout<<cur-1<<nl;
    }
    return 0;
}
/**
