...
#endif
typedef unsigned long long ll; typedef int8_t byte; typedef long double lld;
...
ll f(ll x)
{
    ...
    if(fib.count(x))
        return fib[x];
    if(fib.count(x-1)&&fib.count(x-2))
        return fib[x]=(fib[x-1]+fib[x-2])%M;
    if(x%2)
        return fib[x]=(f(x/2)*f(x/2)+f(x/2+1)*f(x/2+1))%M;
    f(x/2-1); f(x/2); f(x/2+1);
    return fib[x]=fib[x/2]*(fib[x/2-1]+fib[x/2+1])%M;
}
