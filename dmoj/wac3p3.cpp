...
int main()
{
    ...
    fori(0,m)
    {
        cin>>com[i].ps>>com[i].pf.ps; com[i].pf.pf=com[i].ps.size();
    }
    sort(com,com+m); reverse(com,com+m);
    fori(0,s.size())
    {
        forj(0,m)
            if(i+com[j].pf.pf<=s.size()&&s.substr(i,com[j].pf.pf)==com[j].ps)
            {
                t+=com[j].pf.ps;
                i+=com[j].pf.pf-1;
                break;
