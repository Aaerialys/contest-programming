...
int main(){
    ...
    cin>>n>>K;
    dp[0][0][0]=1;
    fori(1,n+1) forj(0,n+1) ford(k,K,0){
        dp[i&1][j][k]=dp[i&1^1][j][k]-dp[i&1^1][j][k+1]+dp[i&1^1][j+1][max(0,k-j)]-dp[i&1^1][j+1][k+1]+dp[i&1][j][k+1];
        if(j) dp[i&1][j][k]+=dp[i&1^1][j-1][k]-dp[i&1^1][j-1][k+1];
        dp[i&1][j][k]=(dp[i&1][j][k]%M+M)%M;
    }
    cout<<dp[n&1][0][K]-dp[n&1][0][K+1];
    return 0;
}
/**
dp[i][j][k]=dp[i-1][j-1][k]+dp[i-1][j+1][k..k-j]
*/