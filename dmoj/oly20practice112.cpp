...
const int N=301,M=MOD,L=8;
ll dp[N][N][N],sub[N],val[N],K,n,m,ans;
string s;
char ch;

ll f(int l,int r,int x){
    if(dp[l][r][x]!=-1) return dp[l][r][x];
    if(l==r) return dp[l][r][x]=-INF*(s[l]-'0'!=x);
    dp[l][r][x]=-INF;
    if((r-l)%(K-1)+1==1){
        foru(y,0,m) if(sub[y]==x){
            ll temp=-INF;
            for(int i=r-1;i>=l;i-=K-1) temp=max(temp,f(l,i,y>>1)+f(i+1,r,y&1));
            dp[l][r][x]=max(dp[l][r][x],temp+val[y]);
        }
        return dp[l][r][x];
    }
    for(int i=r-1;i>=l;i-=K-1) dp[l][r][x]=max(dp[l][r][x],f(l,i,x>>1)+f(i+1,r,x&1));
    return dp[l][r][x];
}
...
int main(){
    ...
    cin>>n>>K>>s; m=P2(K);
    seto(dp,-1);
    fori(0,m) cin>>sub[i]>>val[i];
    fori(0,P2((n-1)%(K-1)+1)) ans=max(ans,f(0,n-1,i));
    cout<<ans;
    return 0;
}
/**
