...
{
    return -(dp[k]-dp[j]+h[k]*h[k]-h[j]*h[j])/2.0/(h[j]-h[k]);
}
...
int main()
{
    ...
    fori(2,n+1)
    {
        while(cb.size()>1&&h[i]>=f(cb[0],cb[1]))
            cb.pop_front();
        dp[i]=dp[cb[0]]+(h[i]-h[cb[0]])*(h[i]-h[cb[0]])+C;
        x=cb[0];
        while(cb.size()>1&&f(cb[cb.size()-2],cb[cb.size()-1])>=f(cb[cb.size()-1],i))
            cb.pop_back();
        cb.pb(i);
    }
    cout<<dp[n];
...
/** dp[j]+(h[j]-h[i])*(h[j]-h[i])>dp[k]+(h[k]-h[i])*(h[k]-h[i])
