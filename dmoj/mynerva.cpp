...
int n,m,q,dist[N],mi[N],ma[N],a,b,c,v;
string s;
vector<pii> gr[N];
queue<int> toV;
...
int main(){
    ...
    cin>>n>>m; seto(dist,inf); seto(mi,inf);
    fori(0,m){
        cin>>a>>b>>c;
        gr[a].eb(b,c); gr[b].eb(a,c);
    }
    cin>>q; dist[1]=mi[1]=0; toV.push(1);
    while(sz(toV)){
        v=toV.front(); toV.pop();
        for(auto i:gr[v]){
            if(dist[v]+1<dist[i.pf]){
                dist[i.pf]=dist[v]+1;
                mi[i.pf]=mi[v]+i.ps;
                ma[i.pf]=ma[v]+i.ps;
                toV.push(i.pf);
            }
            else if(dist[v]+1==dist[i.pf]){
                mi[i.pf]=min(mi[i.pf],mi[v]+i.ps);
                ma[i.pf]=max(ma[i.pf],ma[v]+i.ps);
            }
        }
    }
    while(q--){
        cin>>a>>s;
        cout<<dist[a]<<" "<<(s=="Black"? mi[a]: ma[a])<<nl;
    }
    return 0;
}
/**
