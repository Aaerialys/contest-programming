...
int go(ll x){
    if(x<0) return inf;
    xs.clear(); p.clear(); seto(bit.bit,0);
    fori(0,n) xs.eb(h[i]+i*x);
    sort(all(xs)); xs.erase(unique(all(xs)),end(xs));
    fori(0,n) p.eb(i*x-h[i],xi(h[i]+i*x));
    sort(all(p));
    for(auto i:p) bit.upd(i.ps,bit.query(i.ps)+1);
    return n-bit.query(N-1);
}
...
int main(){
    ...
    while(T--){
        cin>>n>>K; ans=inf;
        fori(0,n) cin>>h[i];
        ford(i,30,0) if(go(ans-P2(i))<=K) ans-=P2(i);
        cout<<ans<<nl;
    }
    return 0;
}
