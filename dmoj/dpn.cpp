...
const int N=410,M=MOD;
ll n,dp[N][N],w[N][N],a[N];
...
int main(){
    ...
    fori(0,n) forj(i+1,n+1){
        if(!i){
            dp[j-i][j]=0; w[j-i][j]=j;
            continue;
        }
        fork(w[j-i][j-1],w[j-i+1][j]+1){
            if(dp[j-i][j]>dp[j-i][k]+dp[k+1][j]){
                dp[j-i][j]=dp[j-i][k]+dp[k+1][j];
                w[j-i][j]=k;
            }
        }
        dp[j-i][j]+=a[j]-a[j-i-1];
    }
    cout<<dp[1][n];
    return 0;
}
