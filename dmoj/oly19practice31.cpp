...
int main(){
    ...
    fori(0,n){
        ...
        stk.push(a[i]);
        tot+=st.query(a[i],a[i])+INF; st.upd(0,a[i],1);
    }
    for(;sz(stk)>1;stk.pop()) lo[stk.top()]=1;
    ans=1; ma=0;
    fori(0,n){
        st.upd(a[i]+1,N-1,1); rem.emplace(-a[i],ma);
        if(a[i]>ma){
            st.upd(a[i],a[i],-st.query(a[i],a[i]));
            ma=a[i];
        }
        if(lo[a[i]]){
            for(;sz(rem)&&-rem.top().pf<a[i];rem.pop()) st.upd(-rem.top().pf+1,rem.top().ps,-1);
            ans=max(ans,st.query(a[i],N-1));
        }
    }
    cout<<tot-ans*2+2;
    return 0;
