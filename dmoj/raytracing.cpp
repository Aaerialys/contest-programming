...
const int inf=0x3f443214,MOD=1e9+7; const ll INF=0x3f3f3f3f3f443214; const lld PI=acos((lld)-1);
...
const int N=8200,M=MOD;
struct BIT{
    int bit[N][N];
    void upd(int x,int y,int v){
        x++; y++;
        for(int i=x;i<N;i+=i&-i) for(int j=y;j<N;j+=j&-j) bit[i][j]+=v;
    }
    int query(int x,int y){
        int ret=0; x++; y++;
        for(int i=x;i;i-=i&-i) for(int j=y;j;j-=j&-j) ret+=bit[i][j];
        return ret;
    }
    int query(int xl,int xr,int yl,int yr){
        return query(xr,yr)-query(xr,yl-1)-query(xl-1,yr)+query(xl-1,yl-1);
    }
} bit;
int n,q,a,b,c,d,e,h[N];

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>n;
    fori(0,n){
        cin>>a;h[i]=a; bit.upd(i,a,1);
    }
    cin>>q;
    fori(0,q){
        cin>>a>>b>>c;
        if(a==1){
            cin>>d>>e;
            cout<<bit.query(b,c,d,e)<<nl;
        }
        else if(a==2){
            bit.upd(b,h[b],-1); h[b]=c; bit.upd(b,h[b],1);
        }
    }
    return 0;
}
