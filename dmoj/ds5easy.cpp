...
struct node{
    int v; sp c[2],p;
    int sz,s,mi,ma,lz,lz2,id;
    bool flip;
    node(int val,int i){
        v=s=mi=ma=val; lz=inf; lz2=0; id=i;
        c[0]=c[1]=p=nullptr; flip=0;
    }
    friend int Sz(sp x){return x?x->sz:0;}
    friend int S(sp x){return x?x->s:0;}
    friend int Mi(sp x){return x?x->mi:inf;}
    friend int Ma(sp x){return x?x->ma:-inf;}
    void upd(int val){
        v=mi=ma=lz=val; s=sz*val; lz2=0;
    }
    void upd2(int val){
        v+=val; mi+=val; ma+=val; s+=sz*val;
        if(lz!=inf) lz+=val;
        else lz2+=val;
    }
    void push(){
