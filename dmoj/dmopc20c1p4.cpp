...
const int N=110,M=MOD;
ll n,K,T,C,f[N][N],x[N][N],temp[N][N],ans,a,b;
ll go(ll n){
    if(n<0) return 1;
    seto(x,0); seto(f,0);
    fori(0,N) x[i][i]=1;
    ...
    f[0][0]=1; f[0][T]=K;
    ford(p,60,0){
        fori(0,N) forj(0,N) temp[i][j]=x[i][j];
        seto(x,0);
        fori(0,N) forj(0,N) fork(0,N) x[i][j]=(x[i][j]+temp[i][k]*temp[k][j])%M;
        if(n&P2(p)){
            fori(0,N) forj(0,N) temp[i][j]=x[i][j];
            seto(x,0);
            fori(0,N) forj(0,N) fork(0,N) x[i][j]=(x[i][j]+temp[i][k]*f[k][j])%M;
        }
    }
    ans=0;
    fori(0,N) ans=(ans+C*x[0][i])%M;
    return ans;
}
...
int main(){
    ...
    cin>>n>>K>>T>>C;
    if(n-1<=T) cout<<C*(2+(n-1)*K)%M;
    else cout<<(go(n-1)+go(n-1-T))%M<<nl;
    return 0;
}
/**
a[0]=C
