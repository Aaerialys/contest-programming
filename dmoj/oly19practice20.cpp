...
const int N=200010,M=MOD,B=1000;//randint(256,M-1);
ll n,m,q,x,y,K,hashnum[N],p[N]={1};
vector<pii> pat[N];
string s;
int hsh(int l,int r){
    return (hashnum[r]-hashnum[l-1]*p[r-l+1]%M+M)%M;
}
void go(){
    cin>>s; s="."+s; n=sz(s); x=y=0;
    fori(1,n) hashnum[i]=(hashnum[i-1]*B+s[i])%M;
    fori(1,n){
        if(y==m){
            x=i; break;
        }
        x=i;
        for(auto j:pat[y]){
            if(hsh(x,x+j.ps-1)==j.pf) x+=j.ps+1;
            else{
                if(y==0){
                    cout<<"NO"<<nl;
                    return;
