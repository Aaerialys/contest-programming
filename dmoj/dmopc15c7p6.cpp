...
const int N=200010,M=1e9+7,V=27;
string in,s;
ll n,pre[N],suf[N],ans,x,last,pal[N];

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>s; n=s.size(); in=s+s+'#';
    fori(0,n){
        in[i*2+1]=s[i]; in[i*2]='#';
    }
    n=in.size();
    ...
    fori(0,n){
        suf[i/2]++; suf[(i+pal[i])/2]--;
        pre[(i-pal[i]+1)/2]++; pre[(i+1)/2]--;
    }
    n=s.size();
    fori(1,n){
        pre[i]+=pre[i-1]; suf[i]+=suf[i-1];
    }
    fori(1,n) suf[i]+=suf[i-1];
    fori(1,n) ans+=suf[i-1]*pre[i];
    cout<<ans;
    return 0;
}
/**
