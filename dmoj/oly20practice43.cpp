...
ll n,m,X,h[N],a,b,c,dist[N],hd[N],v,w,hh,ans;
vector<pii> gr[N];
...
int main(){
    ...
    seto(dist,inf); dist[1]=0; hd[1]=X; toV.emplace(0,1);
    while(sz(toV)){
        tie(w,v)=toV.top(); toV.pop();
        if(-w>dist[v]) continue;
        for(auto i:gr[v]){
            if(i.ps>h[v]) continue;
            w=i.ps; hh=hd[v]-i.ps;
            if(hh<0){
                w-=hh;
                hh=0;
    ...
    if(dist[n]+h[n]-hd[n]>1e16) cout<<-1;
    else cout<<dist[n]+h[n]-hd[n];
    return 0;
}
/**
