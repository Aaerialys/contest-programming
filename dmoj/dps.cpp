...
int n,d[N],dp[N][101],D;
string in;
int f(int pos,int sum,bool limit){
    if(pos==n) return sum==0;
    if(!limit&&dp[pos][sum]!=-1) return dp[pos][sum];
    int mx=limit? d[pos]:9; ll temp=0;
    fori(0,mx+1) temp+=f(pos+1,(sum+i)%D,limit&&i==mx);
    if(!limit) dp[pos][sum]=temp%M;
    return temp%M;
