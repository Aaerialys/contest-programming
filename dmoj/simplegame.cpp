...
const int N=1000010;
int n,q,arr[N],lmax[N],rmax[N],a,b;
...
int main()
{
    ...
    fori(0,n)
        cin>>arr[i];
    lmax[0]=0; rmax[n-1]=n-1;
    fori(1,n)
        lmax[i]=(arr[lmax[i-1]]<arr[i]? i:lmax[i-1]);
    ford(i,n-2,0)
        rmax[i]=(arr[rmax[i+1]]<arr[i]? i:rmax[i+1]);
    fori(0,q)
    {
        cin>>a; a--;
        cout<<lmax[a-1]+1<<" "<<rmax[a+1]+1<<"\n";
    }
    return 0;
}
/**
