...
int n,a,b,c,K,dp[N][3],ans;
vector<pair<int,pii>> gr[N];
bool sm[N];
void dfs(int v,int p=1){
    int m1,m2,m3; m1=m2=m3=0;
    for(auto i:gr[v]) if(i.pf!=p){
        dfs(i.pf,v);
        if(i.ps.ps){
            dp[v][1]+=dp[i.pf][1]+2*i.ps.pf;
            m2=max(m2,min(m1,dp[i.pf][0]-dp[i.pf][1]-i.ps.pf));
            m1=max(m1,dp[i.pf][0]-dp[i.pf][1]-i.ps.pf);
            m3=max(m3,dp[i.pf][2]-dp[i.pf][1]);
        }
        else{
            m2=max(m2,min(m1,dp[i.pf][0]+i.ps.pf));
            m1=max(m1,dp[i.pf][0]+i.ps.pf);
        }
    }
    dp[v][0]=dp[v][1]+m1;
    dp[v][2]=max(dp[v][1]+m3,dp[v][1]+m1+m2);
    ans=max(ans,dp[v][2]);
}
...
int main(){
    ...
    fori(1,n){
        cin>>a>>b>>c;
        gr[a].eb(b,(pii){c,sm[i]}); gr[b].eb(a,(pii){c,sm[i]});
    }
    dfs(1);
    cout<<ans;
    return 0;
...
dp[n][3]
0=longest path up
1=longest down then up
2=longest up then down
*/