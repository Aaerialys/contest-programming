...
const int N=0,M=MOD;
int T;
double p1,p2,p3,u1,u2,u3,theta,d,co,si,x,y,z;
...
int main(){
    ...
    cin>>T;
    while(T--){
        cin>>p1>>p2>>p3>>u1>>u2>>u3>>theta;
        d=sqrt(u1*u1+u2*u2+u3*u3);
        u1/=d; u2/=d; u3/=d; si=sin(theta); co=cos(theta);
        x=p1*(co+u1*u1*(1-co))+p2*(u1*u2*(1-co)-u3*si)+p3*(u1*u3*(1-co)+u2*si);
        y=p1*(u2*u1*(1-co)+u3*si)+p2*(co+u2*u2*(1-co))+p3*(u2*u3*(1-co)-u1*si);
        z=p1*(u3*u1*(1-co)-u2*si)+p2*(u3*u2*(1-co)+u1*si)+p3*(co+u3*u3*(1-co));
        printf("%.8f %.8f %.8f\n",x,y,z);
    }
    return 0;
}
