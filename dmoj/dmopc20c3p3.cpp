...
ll calc(ll a,ll b,ll c){
    return a*(fpow(b,c)-1)%M*fpow(b-1,M-2)%M;
}
...
int main(){
    ...
    while(q--){
        cin>>n>>m>>K>>B;
        if(K==0&&n%4==0) cout<<calc(m*(fpow(B,2)+fpow(B,3))%M,fpow(B,4),n/4);
        else if(K==1&&n%3==0) cout<<calc(m*fpow(B,2)%M,fpow(B,3),n/3);
        else if(K==2&&n%2==0&&m%2==0) cout<<calc(m/2*B%M,fpow(B,2),n/2);
        else if(m%(K+2)==0) cout<<calc(m/(K+2),B,n);
        else cout<<-1;
        cout<<nl;
    }
    return 0;
...
if M%(K+2)==0 place same in every bucket
place

K*a[i]+a[i-1]+a[i+1]=K*a[i+1]+a[i]+a[i+2] = M
