...
const int N=150010,M=MOD,L=9;
int n,a,b,c,len;
vector<str> corp[L];
gp_hash_table<pair<str,int>,bool,chash> ans;
str s;
bool check(str s,int no){
    for(auto i:corp[len]){
        bool pos=1;
        forj(0,len) if(s[j]!='*'&&s[j]!=i[j]||no&P2(i[j]-'A')) pos=0;
        if(pos) return 1;
    }
    return 0;
}
...
bool go(str s,int no,int yes){ //1 if A can win, 0 if B wins
    if(ans.find(mp(s,no))!=ans.end()) return ans[mp(s,no)];
    if(!check(s,no)) return ans[mp(s,no)]=0;
    if(popcount(no)>sz(s)) return ans[mp(s,no)]=1;
    if(done(s)) return ans[mp(s,no)]=0;
    fori(0,26){
        if((P2(i)&no)||(P2(i)&yes)||go(s,no^P2(i),yes)) continue;
        bool pos=1;
        forj(0,len) if(s[j]=='*'){
            s[j]='A'+i;
            if(go(s,no,yes^P2(i))) pos=0;
            s[j]='*';
            if(!pos) break;
        }
        if(pos) return ans[mp(s,no)]=0;
    }
    return ans[mp(s,yes)]=1;
}
int go2(){
    fori(0,L) corp[i].clear();
    cin>>n;
    fori(0,n){
        cin>>s;
        corp[sz(s)].pb(s);
    ...
    fori(0,L){
        len=i; ans.clear();
        if(go(str(len,'*'),0,0)) return cout<<"Yes"<<nl,0;
    }
    cout<<"No"<<nl;
    return 0;
}
...
int main(){
    cin.tie(0)->sync_with_stdio(0); //str f="07"; fileIn(f+".in");
    int T;
    cin>>T;
    while(T--) go2();
    return 0;
...
/**
only consider equal length string
find all 2^7 subsets of all 1000 strings
link one node to another through edge (letter, pos)
from one node, A always chooses highest-move pos of each letter
B will ask each letter from highest to lowest moves
+ store letters at the bottom
