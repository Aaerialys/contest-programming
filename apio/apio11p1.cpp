...
const int inf=0x3f3f3f3f,MOD=1e9; const ll INF=0x3f3f3f3f3f3f3f3f; const lld PI=acos((lld)-1);
...
int main(){
    ...
    fori(0,K){
        cin>>x>>y>>v; v^=x&1&&y&1;
        if(v) dsu.link(x,y+n),dsu.link(x+2e5,y+n+2e5);
        else dsu.link(x,y+n+2e5),dsu.link(x+2e5,y+n);
    }
    fori(1,n+m+1){
        if(dsu.head(i)==dsu.head(i+2e5)) return cout<<0,0;
        ans+=(dsu.head(i)==i);
    }
    cout<<fpow(2,ans-1);
    return 0;
