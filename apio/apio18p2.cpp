...
int n,ans[N],a,b,d,e,k;
ll x[N],y[N],r[N];
pii c[N];
bool dist(ll x,ll y,ll r){
    return x*x+y*y<=r*r;
}

int main(){
    cin.tie(0)->sync_with_stdio(0); //fileIn("38");
    cin>>n;
    fori(1,n+1){
        cin>>x[i]>>y[i]>>r[i]; c[i-1]=mp(-r[i],i);
    }
    sort(c,c+n);
    for(int R=P2(30);R;R/=2){
        vector<tuple<int,int,int>> cir;
        fori(1,n+1) if(!ans[i]) cir.eb(floor(1.0*x[i]/R),floor(1.0*y[i]/R),i);
        sort(all(cir));
        for(;a<=n&&-c[a].pf>=R/2;a++) if(!ans[c[a].ps]){
            b=c[a].ps;
            ans[b]=b;
            fori(-2,3){
                int xx=lower_bound(all(cir),make_tuple(floor(1.0*x[b]/R)+i,floor(1.0*y[b]/R)-2,-inf))-begin(cir);
                for(;xx<sz(cir);xx++){
                    tie(d,e,k)=cir[xx];
                    if(d!=floor(1.0*x[b]/R)+i||e>floor(1.0*y[b]/R)+2) break;
                    if(!ans[k]&&dist(x[b]-x[k],y[b]-y[k],r[b]+r[k])) ans[k]=b;
                }
            }
        }
    }
