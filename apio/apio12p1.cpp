...
struct DSU{
    ...
    bool link(int x,int y){
        x=head(x); y=head(y);
        if(x==y) return 0;
        if(sz(top[x])<sz(top[y])) swap(x,y);
        p[x]+=p[y]; p[y]=x; sum[x]+=sum[y];
        for(;sz(top[y]);top[y].pop()) top[x].em(top[y].top());
        while(sum[x]>B){
            sum[x]-=top[x].top(); top[x].pop();
        }
        return 1;
    }
}dsu;
...
int main(){
    ...
    cin>>n>>B;
    fori(1,n+1){
        cin>>p[i]>>c[i]>>l[i]; sum[i]=c[i]; top[i].em(c[i]);
    }
    ford(i,n,1){
        ckmax(ans,1LL*sz(top[dsu.head(i)])*l[i]);
        dsu.link(i,p[i]);
    }
    cout<<ans;
    return 0;
