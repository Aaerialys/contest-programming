...
const int N=2010,M=MOD;
ll n,m,a,b,dp[N][N],inv[N],len,ans,comb;
map<int,int> cord;
vector<int> poi;
pii lr[N];
...
int main(){
    ...
    fori(1,N) inv[i]=fpow(i,M-2);
    cin>>n;
    fori(1,n+1){
        cin>>lr[i].pf>>lr[i].ps; lr[i].ps++; poi.pb(lr[i].pf); poi.pb(lr[i].ps);
    }
    poi.pb(0); sort(all(poi)); poi.erase(unique(all(poi)),poi.end());
    for(auto i:poi) cord[i]=a++;
    fori(1,n+1){
        lr[i].pf=cord[lr[i].pf]; lr[i].ps=cord[lr[i].ps];
    }
    m=sz(poi)-1;
    fori(0,m) dp[i][0]=1;
    fori(1,m){
        len=poi[i+1]-poi[i];
        forj(1,n+1){
            a=1; comb=len; dp[i][j]=dp[i-1][j];
            if(lr[j].pf<=i&&i<lr[j].ps) ford(k,j-1,0){
                dp[i][j]=(dp[i][j]+dp[i-1][k]*comb)%M;
                if(lr[k].pf<=i&&i<lr[k].ps){
                    a++;
                    comb=comb*(len+a-1)%M*inv[a]%M;
                }
            }
        }
    }
    fori(1,n+1) ans+=dp[m-1][i];
    cout<<ans%M;
    return 0;
}
/**
