...
const ll N=400010,M=P2(61)-1,B=randll(300,M-1);
...
ll n,m,hs[N],ht[N],p[N]={1},x,ans;
string s,t;
gp_hash_table<ll,null_type,chash> cyc;
ll hsh(int l,int r,ll* hn){
    return (hn[r]-mul(hn[l-1],p[r-l+1])+M)%M;
}

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>t>>s; n=sz(t); m=sz(s); t="."+t; s="."+s+s;
    fori(1,N) p[i]=mul(p[i-1],B);
    fori(1,n+1) ht[i]=(mul(ht[i-1],B)+t[i])%M;
    fori(1,m*2+1) hs[i]=(mul(hs[i-1],B)+s[i])%M;
    fori(1,m+1) cyc.insert(hsh(i,i+m-1,hs));
    fori(m,n+1) if(cyc.find(hsh(i-m+1,i,ht))!=cyc.end()){
        cout<<"yes";
        return 0;
    }
    cout<<"no";
