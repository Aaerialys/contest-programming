...
int main(){
    ...
    //freopen("d2p1.12.in", "r", stdin); //freopen("", "w", stdout);
    cin>>n;
    fori(2,n+1){
        cin>>in;
        forj(1,i) gr[i][j]=gr[j][i]=(in[j-1]=='R');
    }
    fori(1,n+1){
        s.push(i);
        forj(1,n+1) if(i!=j){
            if(!sz(t)) t.push(j);
                ...
                x=s.top(); y=t.top();
                if(gr[x][j]==sc) s.push(j);
                else if(gr[j][y]!=sc) t.push(j);
                else if(gr[x][y]==sc){
                    s.push(y); s.push(j); t.pop();
                }
                else{
                    if(x!=i){
                        t.push(x); t.push(j); s.pop();
                    }
                    else{
                        sc^=1;
                        for(;sz(t);t.pop()) s.push(t.top());
                        t.push(j);
                    }
                }
            }
        }
        for(;sz(s);s.pop()) t.push(s.top());
        cout<<sz(t)<<nl;
        for(;sz(t);t.pop()) cout<<t.top()<<" ";
        cout<<nl;
    }
    return 0;
}
