...
struct DSU{
    ...
    bool link(int x,int y){
        x=head(x); y=head(y); if(x==y) return 0;
        if(p[x]>p[y]) swap(x,y);
        p[x]+=p[y]; p[y]=x; return 1;
    }
} dsu;
ll n,m,x[N],y[N],t[N],c[N],T,C;
bitset<N> link;
vector<pii> edge;
pii mst(pii w){
    pii ret; dsu.clear(); edge.clear();
    fori(0,m) edge.eb(w.pf*t[i]+w.ps*c[i],i);
    sort(all(edge));
    for(auto i:edge) if(dsu.link(x[i.ps],y[i.ps])){
        ret.pf+=t[i.ps],ret.ps+=c[i.ps];
        link[i.ps]=1;
    }
    if(ret.pf*ret.ps<T*C) tie(T,C)=ret;
    return ret;
}
void go(pii l,pii r){
    pii mid=mst(mp(l.ps-r.ps,r.pf-l.pf));
...
int main(){
    ...
    cin>>n>>m; T=C=inf;
    fori(0,m) cin>>x[i]>>y[i]>>t[i]>>c[i];
    go(mst(mp(1,0)),mst(mp(0,1)));
    link.reset(); mst(mp(C,T));
    cout<<T<<" "<<C<<nl;
    fori(0,m) if(link[i]) cout<<x[i]<<" "<<y[i]<<nl;
    return 0;
}
/**
