...
int main(){
    ...
    cin>>n;
    fori(1,32000) if(n%i==0) d.pb(i),d.pb(n/i);
    sort(all(d)); d.erase(unique(all(d)),end(d));
    ans[0].insert(0); n=sz(d);
    fori(1,n) forj(0,i) if(d[i]%d[j]==0) for(auto k:ans[j]) ans[i].insert(k+d[i]/d[j]-1);
    cout<<sz(ans[n-1])<<nl;
    for(auto i:ans[n-1]) cout<<i<<" ";
    return 0;
}
/**
