...
int main(){
    ...
    cin>>n;
    fori(0,n){
        cin>>a>>b>>c; it.eb(b,a,-c);
    }
    cin>>m;
    fori(0,m){
        cin>>a>>b>>c; it.eb(b,-a,c);
    }
    n+=m; sort(all(it)); reverse(all(it));
    seto(dp,-inf); dp[1][0]=0;
    for(auto xx:it){
        tie(c,a,b)=xx; swap(dp[0],dp[1]);
        fori(0,M){
            dp[1][i]=dp[0][i];
            if(0<=i-a&&i-a<M) dp[1][i]=max(dp[1][i],dp[0][i-a]+b);
        }
    }
    fori(0,M) ans=max(ans,dp[1][i]);
    cout<<ans;
    return 0;
}
/**
