...
const ll N=10010;
int n,q,a[N],ans[101][N],x,y,L;
vector<int> que,qc;
int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>n>>L;
    fori(1,n+1) cin>>a[i];
    cin>>q; que.resize(q); qc.resize(q);
    fori(0,q) cin>>que[i],qc[i]=que[i];
    sort(all(qc)); x=0; n-=L-1;
    fori(1,n){
        x=0;
        forj(0,L) x+=(a[j]!=a[i+j]);
        forj(0,n-i){
            x+=(a[j+L]!=a[i+j+L])-(a[j]!=a[i+j]);
            y=lower_bound(all(qc),x)-begin(qc);
            ans[y][j]++; ans[y][i+j]++;
        }
    }
    fori(1,101) forj(0,n) ans[i][j]+=ans[i-1][j];
    for(auto i:que){
        x=lower_bound(all(qc),i)-begin(qc);
        forj(0,n) cout<<ans[x][j]<<" ";
        cout<<nl;
    }
    return 0;
}
