...
const int N=410,D=21,dn[6]={4,6,8,10,12,20};
double dp[D][D][N],ans;
int T,h,n,x,y,z;
string in;
...
int main(){
    ...
    //freopen("fighting_the_zombie.txt", "r", stdin); //freopen("", "w", stdout);
    for(auto V:dn){
        dp[V][0][0]=1;
        fori(1,D) forj(1,V+1) fork(j,N) dp[V][i][k]+=dp[V][i-1][k-j]/V;
        fori(1,D) ford(j,N-2,0) dp[V][i][j]+=dp[V][i][j+1];
    }
    cin>>T;
    foru(cnt,1,T+1){
        cin>>h>>n; ans=0;
        while(n--){
            cin>>in; z=0;
            fori(0,in.size()){
                if(in[i]=='+') z=stoi(in.substr(i+1));
                if(in[i]=='-') z=-stoi(in.substr(i+1));
                if(i==in.size()-1&&in[i]!='#') in+="#";
                if(in[i]=='+'||in[i]=='-'||in[i]=='#'){
                    x=stoi(in.substr(x,i-x));
                    break;
                }
                if(in[i]=='d'){
                    y=stoi(in.substr(0,i));
                    x=i+1;
                }
            }
            ans=max(ans,dp[x][y][max(0,min(h-z,N-1))]);
        }
        printf("Case #%d: %.8f\n",cnt,ans);
    }
    return 0;
