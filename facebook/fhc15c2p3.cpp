...
const int N=20010,MK=401,V=26,M=1e9+7;
int T,n,K,nex[N][V],dep[N],dp[N][MK];
string s;
bool we[N];
struct Trie{
    int n;
...
void dfs(int v){
    dp[v][0]=0;
    if(we[v]) dp[v][1]=dep[v];
    for(auto i:nex[v]) if(i){
        dep[i]=dep[v]+1; dfs(i);
        ford(j,MK-1,1) fork(0,j+1) dp[v][j]=min(dp[v][j],dp[v][k]+dp[i][j-k]);
    }
    dp[v][1]=dep[v];
}
...
int main(){
    ...
    cin>>T;
    foru(cnt,1,T+1){
        cin>>n>>K; seto(dp,inf); seto(nex,0); tr.n=0; seto(we,0);
        fori(0,n){
            cin>>s; tr.ins(s);
        }
        dfs(0);
