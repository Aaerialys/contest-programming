...
const int N=20010,MK=101,V=26,M=1e9+7;
int T,n,K,nex[N][V],dep[N],dp[N][MK],sz[N];
string s;
struct Trie{
    int n;
    void ins(){
        int cur=0;
        for(auto i:s){
            if(!nex[cur][i-'a']) nex[cur][i-'a']=++n;
            dep[nex[cur][i-'a']]=dep[cur]+1;
            cur=nex[cur][i-'a'];
        }
        sz[cur]=1;
    }
...
int main(){
    ...
    cin>>T;
    foru(cnt,1,T+1){
        cin>>n>>K; seto(dp,inf); seto(nex,0); tr.n=0; seto(sz,0);
        fori(0,n){
            cin>>s; tr.ins();
        }
        ford(v,tr.n,0){
            dp[v][0]=0;
            if(sz[v]) dp[v][1]=dep[v];
            for(auto i:nex[v]) if(i){
                sz[v]+=sz[i];
                ford(j,min(K,sz[v]),1) fork(0,j+1) dp[v][j]=min(dp[v][j],dp[v][k]+dp[i][j-k]);
            }
            dp[v][1]=dep[v];
        }
        cout<<"Case #"<<cnt<<": "<<dp[0][K]<<"\n";
    }
    return 0;
}
/**
