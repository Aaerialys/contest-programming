...
int main(){
    ...
    cin>>n>>m>>a>>b; n++; S=a+n*b;
    cin>>a>>b; t=a+n*b;
    fori(0,m){
        cin>>s;
        forj(0,n-1){
            if(s[j]=='w')
                gr[0][i*n+j].pb(i*n+j+1),gr[0][i*n+j+1].pb(i*n+j);
            if(s[j]=='b')
                gr[1][i*n+j].pb(i*n+j+1),gr[1][i*n+j+1].pb(i*n+j);
        }
        cin>>s;
        forj(0,sz(s)){
            if(s[j]=='w')
                gr[0][i*n+j/2+(j&1)].pb(i*n+n+j/2),gr[0][i*n+n+j/2].pb(i*n+j/2+(j&1));
            if(s[j]=='b')
                gr[1][i*n+j/2+(j&1)].pb(i*n+n+j/2),gr[1][i*n+n+j/2].pb(i*n+j/2+(j&1));
        }
    }
    cin>>s;
    forj(0,n-1){
        if(s[j]=='w')
            gr[0][m*n+j].pb(m*n+j+1),gr[0][m*n+j+1].pb(m*n+j);
        if(s[j]=='b')
            gr[1][m*n+j].pb(m*n+j+1),gr[1][m*n+j+1].pb(m*n+j);
    }
    seto(d,inf); d[0][S]=d[1][S]=0; toV.emplace(0,S); toV.emplace(1,S);
    while(sz(toV)){
        tie(b,a)=toV.front(); toV.pop();
        for(auto i:gr[b^1][a]) if(d[b^1][i]>d[b][a]+1){
            d[b^1][i]=d[b][a]+1;
            toV.emplace(b^1,i);
        }
    }
    cout<<min(d[0][t],d[1][t]);
    return 0;
}
/**
