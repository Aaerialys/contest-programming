...
int n,a,b,c,x,dp[N][2],ans;
vector<int> gr[N];
bitset<N> vis,cur;
gp_hash_table<string,int,chash> id;
string s1,s2;
void dfs(int v,int p=0){
    vis[v]=cur[v]=1;
    for(auto i:gr[v]) if(!cur[i]){
        if(!vis[i]) dfs(i,v);
        dp[v][1]+=dp[i][0]; dp[v][0]=max(dp[v][0],dp[i][1]-dp[i][0]);
    }
    dp[v][0]+=dp[v][1]; dp[v][1]++;
    for(auto i:gr[v]) if(cur[i]){
        a=v,b=p;
        if(i==p) c=1;
    }
...
void dfs2(int v){
    vis[v]=dp[v][0]=dp[v][1]=0;
    for(auto i:gr[v]) if(vis[i]) dfs2(i);
}
...
int main(){
    ...
    fori(1,n+1) if(!vis[i]){
        ...
        if(a){
            x=dp[i][0]; c=0; dfs2(i);
            forj(0,sz(gr[a])) if(gr[a][j]==b) gr[a].erase(begin(gr[a])+j);
            forj(0,sz(gr[b])) if(gr[b][j]==a) gr[b].erase(begin(gr[b])+j);
            dfs(a); c=dp[a][0];
            if(!vis[b]) dfs(b),c=dp[a][1]+dp[b][1];
            x=max(x,c);
            ans+=x; a=0;
        }
    }
