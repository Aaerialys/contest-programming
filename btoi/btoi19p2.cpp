...
const int N=510,M=MOD;
int n,m,q,x,ans;
string in;
bitset<N> pos[2][N],mp[N];
char ch;
...
int main(){
    ...
    fori(1,n+1) forj(1,m+1){
        cin>>ch; pos[x][i][j]=mp[i][j]=ch=='.';
    }
    cin>>in;
    for(auto c:in){
        x^=1;
        if(c=='N') fori(1,n+1) pos[x][i]=pos[x^1][i+1]&mp[i];
        if(c=='S') fori(1,n+1) pos[x][i]=pos[x^1][i-1]&mp[i];
        if(c=='E') fori(1,n+1) pos[x][i]=(pos[x^1][i]<<1)&mp[i];
        if(c=='W') fori(1,n+1) pos[x][i]=(pos[x^1][i]>>1)&mp[i];
        if(c=='?') fori(1,n+1) pos[x][i]=(pos[x^1][i+1]|pos[x^1][i-1]|(pos[x^1][i]<<1)|(pos[x^1][i]>>1))&mp[i];
    }
    fori(1,n+1) ans+=pos[x][i].count();
    cout<<ans;
    return 0;
}
/**
