...
const int N=310,M=91010;
int n,m,K,ax,a,b,c,ans,dp[M];

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    //freopen("", "r", stdin); //freopen("", "w", stdout);
    cin>>n>>m>>K; ans=inf; seto(dp,-inf); dp[0]=0;
    fori(0,n){
        cin>>a;
        if(a<K){
            cout<<"Impossible";
    ...
    fori(0,m){
        cin>>a;
        ford(j,M-1,a) dp[j]=max(dp[j],dp[j-a]+min(n,a));
    }
    fori(ax,M) if(dp[i]>=n*K) ans=min(ans,i-ax);
