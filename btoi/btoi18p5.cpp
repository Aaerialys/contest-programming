...
const int N=4200,M=MOD;
ull n,m,K,x,t,sum[N][4],hsh[N];
str s[N],dna="ACGT";
...
int main(){
    ...
    cin>>n>>m>>K;
    fori(0,n){
        cin>>s[i]; hsh[i]=randll(0,INF); t+=hsh[i];
        forj(0,m) fork(0,4) if(s[i][j]==dna[k]) sum[j][k]+=hsh[i];
    }
    fori(0,n){
        x=0;
        forj(0,m) fork(0,4) if(s[i][j]==dna[k]) x+=t-sum[j][k];
        if(x==(t-hsh[i])*K){
            cout<<i+1<<nl;
            return 0;
        }
