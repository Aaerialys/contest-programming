...
bool gr[N][N];
int n,m,a,b,d,dp[N][N][2],p[N][N][2],c;
bool f(int a,int b,int d){
    if(a==b) return 1;
    if(dp[a][b][d]!=-1) return dp[a][b][d];
    dp[a][b][d]=0;
    if(d){
        if(gr[(b-1+n)%n][b]&&f(a,(b-1+n)%n,1)){
            p[a][b][d]=0; dp[a][b][d]=1;
        }
        else if(gr[a][b]&&f(a,(b-1+n)%n,0)){
            p[a][b][d]=1; dp[a][b][d]=1;
        }
    }
    else{
        if(gr[(a+1)%n][a]&&f((a+1)%n,b,0)){
            p[a][b][d]=0; dp[a][b][d]=1;
        }
        else if(gr[a][b]&&f((a+1)%n,b,1)){
            p[a][b][d]=1; dp[a][b][d]=1;
        }
    }
    return dp[a][b][d];
...
int main(){
    cin.tie(0)->sync_with_stdio(0); //fileIn("mexico3.in");
    cin>>n>>m; seto(dp,-1);
    fori(0,m){
        cin>>a>>b; a--; b--; gr[a][b]=gr[b][a]=1;
    }
    fori(0,n){
        d=-1; b=i; a=(i+1)%n;
        if(f(a,b,0)&&gr[a][b]) d=0;
        if(f(a,b,1)&&gr[a][b]) d=1;
        if(d!=-1){
            while(a!=b){
                cout<<(d? b: a)+1<<endl; c=d;
                if(p[a][b][d]) d^=1;
                if(c) b=(b-1+n)%n;
                else a=(a+1)%n;
            }
