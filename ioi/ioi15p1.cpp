...
const int N=10000010,M=MOD;
ll n,K,L,ans,to[N],pr[N];
int p[N];
long long delivery(int _N,int _K,int _L,int _p[]){
    n=_N; K=_K; L=_L;
    fori(1,n+1) p[i]=_p[i-1];
    fori(1,n+1) to[i]=to[max(0,i-K)]+p[i]*2;
    ford(i,n,0) pr[i]=pr[min(n+1,i+K)]+(L-p[i])*2;
    ans=pr[K]+L;
    fori(K,n+1) ans=min(ans,pr[i+1]+to[i-K]+L);
    fori(1,n+2) ans=min(ans,to[i-1]+pr[i]);
    return ans;
}
