...
const int N=260,M=MOD;
int n,m,q,a,b,c,e[N][N],d[N][N],s[N],ans;
vector<int> t[N],gr[N];
queue<int> toV;
...
int main(){
    ...
    fori(0,q) cin>>s[i];
    fori(1,m+1){
        cin>>a; vector<int> x(a);
        forj(0,a) cin>>x[j];
        forj(0,a){
            t[x[j]].pb(i);
            if(e[x[(j+1)%a]][x[j]]){
                gr[i].pb(e[x[(j+1)%a]][x[j]]); gr[e[x[(j+1)%a]][x[j]]].pb(i); e[x[(j+1)%a]][x[j]]=0;
            }
            else e[x[j]][x[(j+1)%a]]=i;
        }
    }
    seto(d,inf); ans=inf;
    fori(0,q){
        for(auto j:t[s[i]]) d[i][j]=0,toV.em(j);
        while(sz(toV)){
            a=toV.front(); toV.pop();
            for(auto j:gr[a]) if(d[i][j]>d[i][a]+1) d[i][j]=d[i][a]+1,toV.em(j);
        }
    }
    fori(1,m+1){
        a=0;
