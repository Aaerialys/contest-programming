...
const int N=5010,M=MOD,L=13;
int ANS[N],S[N];
int n,ans[N],s[N],x,pre;
#ifdef LOCAL_PROJECT
int tryCombination(int* arr){
    int ret=inf;
    fori(0,n) if(arr[S[i]]!=ANS[S[i]]) ckmin(ret,i);
    if(ret==inf) ret=-1;
    return ret;
}
void answer(int *s,int *ans){
    fori(0,n) if(ans[i]!=ANS[i]) dbg(i,ans[i],ANS[i]);
    cout<<nl;
    fori(0,n) if(s[i]!=S[i]) dbg(i,s[i],S[i]);
}
#endif
int query(int r,int x){
    int arr[N]={0}; r=min(r,n);
    fori(0,r) arr[i]=1;
    fori(0,n) if(ans[i]!=-1) arr[i]=ans[i];
...
int main(){
    cin.tie(0)->sync_with_stdio(0); fileIn("known-combo-interleaved-5000.in");
    cin>>n;
    fori(0,n) cin>>ANS[i];
    fori(0,n) cin>>S[i];
    exploreCave(n);
