...
const int N=3010,M=MOD;
int n,m,x,h,w,b,c,v[N][N],l,s[N][N];
bool go(int x){
    seto(s,0);
    fori(1,n+1) forj(1,m+1) if(v[i][j]>x){
        s[max(1,i-h+1)][max(1,j-w+1)]++; s[max(1,i-h+1)][j+1]--; s[i+1][max(1,j-w+1)]--; s[i+1][j+1]++;
    }
    fori(1,n+1-h+1) forj(1,m+1-w+1){
        s[i][j]+=s[i-1][j]+s[i][j-1]-s[i-1][j-1];
        if(s[i][j]*2<h*w) return 1;
    }
    return 0;
}
int rectangle(int _n,int _m,int _h,int _w,int _q[3001][3001]){
    n=_n; m=_m; h=_h; w=_w;
    fori(1,n+1) forj(1,m+1) v[i][j]=_q[i-1][j-1];
    x=N*N;
    ford(i,24,0){
        if(go(x-P2(i))) x-=P2(i);
    }
...
int main(){
    ...
    cin>>n>>m>>h>>w; int q[3001][3001];
    fori(0,n) forj(0,m) cin>>q[i][j];
