...
struct SegTree{
    typedef pair<lld,ll> T;
    struct node{
        T v=mp(0,1),lz=mp(0,1);
    } seg[2*N];
    void pull(int x){
        seg[x].v=max(seg[2*x].v,seg[2*x+1].v);
    }
    void push(int x){
        if(!seg[x].lz.pf) return;
        if(x<N) fori(0,2) seg[2*x+i].lz.pf+=seg[x].lz.pf,seg[2*x+i].lz.ps=seg[2*x+i].lz.ps*seg[x].lz.ps%M;
        seg[x].v.pf+=seg[x].lz.pf; seg[x].v.ps=seg[x].v.ps*seg[x].lz.ps%M;
        seg[x].lz=mp(0,1);
    }
    void upd(int lo,int hi,T v,int l=0,int r=N-1,int x=1){
        push(x);
        if(r<lo||hi<l) return;
        if(lo<=l&&r<=hi){
            seg[x].lz.pf+=v.pf; seg[x].lz.ps=seg[x].lz.ps*v.ps%M; push(x);
            return;
        }
        int mid=l+r>>1;
...
int init(int _N,int X[],int Y[]){
    ...
    fori(0,n){
         x[i]=X[i]; y[i]=Y[i];
         st.upd(i,n,mp(log(x[i]),x[i])); st.upd(i,i,mp(log(y[i]),y[i]));
    }
    return st.query();
}
int updateX(int pos,int val){
    st.upd(pos,n,mp(log(val)-log(x[pos]),val*fpow(x[pos],M-2)%M));
    x[pos]=val;
    return st.query();
}
int updateY(int pos,int val){
    st.upd(pos,pos,mp(log(val)-log(y[pos]),val*fpow(y[pos],M-2)%M));
    y[pos]=val;
    return st.query();
}

int main(){
    cin.tie(0)->sync_with_stdio(0); fileIn("01");
    cin>>n; int x[n],y[n],q,a,b,c;
    fori(0,n) cin>>x[i];
    fori(0,n) cin>>y[i];
    cout<<init(n,x,y)<<nl;
    cin>>q;
    fori(0,q){
        cin>>a>>b>>c;
        if(a==1) cout<<updateX(b,c)<<nl;
        else cout<<updateY(b,c)<<nl;
    }
    return 0;
}
/**
