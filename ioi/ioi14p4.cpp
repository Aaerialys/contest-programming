...
const int inf=0x3f3f3f3f,MOD=1e9+9; const ll INF=0x3f3f3f3f3f3f3f3f; const lld PI=acos((lld)-1);
...
const int N=250010,M=MOD;
ll n,a,b,c;
int valid(int n,int x[]){
    a=-1;
    fori(0,n) if(x[i]<=n){
        if(a==-1) a=(x[i]-i+n)%n;
        else if((x[i]-i+n)%n!=a) return 0;
    }
    bitset<N> f;
    fori(0,n){
        if(f[x[i]]) return 0;
...
int replacement(int n,int x[],int ans[]){
    vector<pii> v;
    fori(0,n) if(x[i]<=n) a=(x[i]-i+n)%n;
    int cur[N];
    fori(0,n) cur[i]=(i+a)%n+((i+a)%n==0)*n;
    b=n; a=0;
    fori(0,n) v.eb(x[i],i);
    sort(all(v));
    for(auto i:v) for(;b<i.pf;b++) {
       ans[a++]=cur[i.ps];
       cur[i.ps]=b+1;
    }
    return a;
...
int main(){
    cin.tie(0)->sync_with_stdio(0); fileIn("10-014.in");
    cin>>n>>n; int x[n];
    fori(0,n) cin>>x[i];
    cout<<countReplacement(n,x);
    return 0;
}
/**
8
