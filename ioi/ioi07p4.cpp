...
int n,x,dp[2][4][4][4][4],ans;
char c;

int main(){
    cin.tie(0)->sync_with_stdio(0);
    cin>>n; seto(dp[1],-inf); dp[1][0][0][0][0]=0;
    while(n--){
        cin>>c; swap(dp[0],dp[1]); seto(dp[1],-inf);
        if(c=='M') x=1;
        else if(c=='F') x=2;
        else if(c=='B') x=3;
        fori(0,4) forj(0,4) fork(0,4) foru(l,0,4){
            ckmax(dp[1][j][x][k][l],dp[0][i][j][k][l]+1+(i!=j||j!=x||i!=x)+(i!=j&&j!=x&&i!=x)-(!i||!j));
            ckmax(dp[1][i][j][l][x],dp[0][i][j][k][l]+1+(k!=l||k!=x||l!=x)+(k!=l&&k!=x&&l!=x)-(!k||!l));
        }
    }
    fori(0,4) forj(0,4) fork(0,4) foru(l,0,4) ckmax(ans,dp[1][i][j][k][l]);
    cout<<ans;
    return 0;
}
/**
