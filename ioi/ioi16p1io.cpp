...
ll a,x,ans[N];
int find_subset(int l,int r,int* w,int n,ll* ans){
    vector<pii> id;
    fori(0,n) id.eb(w[i],i); sort(all(id)); sort(w,w+n);
    fori(0,n){
        x+=w[i];
        for(;x>r;a++) x-=w[a];
        if(x>=l){
            x=0;
            forj(a,i+1) ans[x++]=id[j].ps;
            return x;
        }
    }
    return 0;
...
int main(){
    cin.tie(0)->sync_with_stdio(0); string s="103";
    int l,r,w[N],n; //fileIn(s.c_str());
    cin>>n>>l>>r;
    fori(0,n) cin>>w[i];
    n=find_subset(l,r,w,n,ans);
    cout<<n<<nl;
    fori(0,n) cout<<ans[i]<<" ";
    return 0;
}
