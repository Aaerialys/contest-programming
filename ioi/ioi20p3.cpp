...
ll n,m,ans,tk[N],tn[N],bn[N],a,b,done[N];
vector<pii> top;

long long find_maximum(int K,vector<vector<int>> v){
    n=sz(v); m=sz(v[0]); vector<vector<int>> ret(n,vector<int>(m,-1));
    fori(0,n) forj(0,K){
        ans-=v[i][j]; top.eb(v[i][sz(v[i])-K+j]+v[i][j],i);
    }
    sort(all(top)) ; reverse(all(top));
    fori(0,n*K/2) ans+=top[i].pf,tk[top[i].ps]++;
    fori(0,n) bn[i]=K-tk[i],tn[i]=tk[i];
    dbg(ret);
    fori(0,K){
        a=b=n/2; seto(done,0);
        forj(0,n){
            if(!tn[j]) ret[j][--bn[j]]=i,a--,done[j]=1;
            else if(!bn[j]) ret[j][sz(v[j])-tk[j]+(--tn[j])]=i,b--,done[j]=1;
        }
        dbg(ret);
        forj(0,n) if(!done[j]){
            if(a) ret[j][--bn[j]]=i,a--;
            else if(b) ret[j][sz(v[j])-tk[j]+(--tn[j])]=i,b--;
        }
        dbg(ret);
    }
...
int main(){
    ...
    int K;
    cin>>n>>m>>K; vector<vector<int>> v(n,vector<int>(m));
    fori(0,n) forj(0,m) cin>>v[i][j];
