...
int n,sum[N],a[N],b[N];
bool dp1[N][M],dp2[N][M],dp3[N][M];
string ans;
string solve_puzzle(string s,int K,int *c){
    n=sz(s); s="."+s+".";
    fori(1,n+2) sum[i]=sum[i-1]+(s[i]=='_');
    dp1[0][0]=dp3[0][0]=1;
    fori(1,n+2) forj(0,K+1) if(s[i]!='X'){
        dp1[i][j]|=dp1[i-1][j];
        if(j&&i>c[j-1]&&sum[i-1]==sum[i-c[j-1]-1]) dp1[i][j]|=dp1[i-c[j-1]-1][j-1],dp3[i-1][j]=dp1[i-c[j-1]-1][j-1];
    }
    dp2[n+1][0]=1; reverse(c,c+K);
    ford(i,n,0) forj(0,K+1) if(s[i]!='X'){
        dp2[i][j]|=dp2[i+1][j];
        if(j&&i+c[j-1]<=n&&sum[i]==sum[i+c[j-1]]) dp2[i][j]|=dp2[i+c[j-1]+1][j-1];
    }
    reverse(c,c+K);
    fori(1,n+1){
        forj(0,K+1) if(dp1[i][j]&dp2[i][K-j]) a[i]=1;
        forj(1,K+1) if(dp3[i][j]&dp2[i+1][K-j]) b[i-c[j-1]+1]++,b[i+1]--;
    }
    fori(1,n+1){
        b[i]+=b[i-1];
        if(s[i]=='.'){
...
int main(){
    cin.tie(0)->sync_with_stdio(0); //fileIn("50");
    string s; int K,c[M];
    cin>>s>>K;
    fori(0,K) cin>>c[i];
