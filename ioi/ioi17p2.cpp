...
long long min_total_length(vector<int> _a,vector<int> _b){
    n=sz(_a)+sz(_b);
    for(auto i:_a) p.eb(i,0);
    for(auto i:_b) p.eb(i,1);
    p.eb(-inf,-1); sort(all(p));
    seto(dp,inf); dp[0]=0;
    nxt[0]=1;
    fori(1,n+1){
        if(p[i].ps==p[i-1].ps) nxt[i]=nxt[i-1];
        else{
            nxt[i]=i; s=0;
            forj(nxt[i-1],i) dp[j]=min(dp[j],dp[j-1]+p[i].pf-p[j].pf);
        }
        dp[i]=dp[i-1]+p[i].pf-p[nxt[i]-1].pf;
        if(nxt[i]*2-1-i>=nxt[nxt[i]-1]){
            s+=p[i].pf-p[nxt[i]*2-1-i].pf;
            dp[i]=min(dp[i],dp[nxt[i]*2-1-i-1]+s);
        }
    }
    return dp[n];
}
...
int main(){
    ...
    fileIn("1-02.in"); string s; cin>>s;
    cin>>n>>m; vector<int> r(n),b(m);
    fori(0,n) cin>>r[i];
    fori(0,m) cin>>b[i];
...
/**
the editorial makes it sound so much more complicated than it is...
*/