...
const int N=810,M=MOD;
int n,S,a,t[N][N],d[N][N],x1,Y1,x2,y2,dx[4]={1,0,-1,0},dy[4]={0,1,0,-1},x,y,ans,T;
char ch;
queue<pii> toV;

int main(){
    cin.tie(0)->sync_with_stdio(0); //fileIn("mecho.033.in");
    cin>>n>>S;
    fori(1,n+1) forj(1,n+1){
        cin>>ch; t[i][j]=inf;
        if(ch=='T') t[i][j]=0;
        else if(ch=='M') x1=i,Y1=j;
        else if(ch=='D') x2=i,y2=j;
        else if(ch=='H'){
            t[i][j]=0;
            toV.em(i,j);
        }
    ...
    while(sz(toV)){
        auto [x,y]=toV.front(); toV.pop();
        if(x==x2&&y==y2) continue;
        fori(0,4) if(t[x+dx[i]][y+dy[i]]>t[x][y]+1){
            t[x+dx[i]][y+dy[i]]=t[x][y]+1;
            toV.em(x+dx[i],y+dy[i]);
        }
    }
    ans=-1; t[x2][y2]=1e6;
    ford(j,20,0){
        T=ans+P2(j);
        if(T>=t[x1][Y1]) continue;
        seto(d,inf); d[x1][Y1]=0; toV.em(x1,Y1);
        while(sz(toV)){
            auto [x,y]=toV.front(); toV.pop();
            fori(0,4) if(min((t[x+dx[i]][y+dy[i]]-T)*S,d[x+dx[i]][y+dy[i]])>d[x][y]+1){
                d[x+dx[i]][y+dy[i]]=d[x][y]+1;
                toV.em(x+dx[i],y+dy[i]);
            }
        }
        if(d[x2][y2]<inf) ans=T;
    }
    cout<<ans;
    return 0;
}
