...
string guess_sequence(int n){
    string ret;
    char c[5]="ABXY";
    if(press("XY")) press("X")? swap(c[0],c[2]): swap(c[0],c[3]);
    else if(press("B")) swap(c[0],c[1]);
    ret=c[0];
    if(n==1) return ret;
    fori(1,n-1){
        int x=press(ret+c[1]+ret+c[2]+c[1]+ret+c[2]+c[2]+ret+c[2]+c[3]);
        if(x==i) ret+=c[3];
        else if(x==i+1) ret+=c[1];
        else if(x==i+2) ret+=c[2];
    }
    if(press(ret+c[1]+ret+c[2])==n) press(ret+c[1])==n? ret+=c[1]: ret+=c[2];
    else ret+=c[3];
    return ret;
}
