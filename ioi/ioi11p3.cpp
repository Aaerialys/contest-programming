...
ll n,L,B,x[N],z[N],ans;
bool go(int len){
    int l=(len-1)/2,r=len/2;
    ll cost=INF;
    fori(l+1,n+1-r) ckmin(cost,l*x[i]-(z[i-1]-z[i-l-1])+z[i+r]-z[i]-r*x[i]);
    return cost<=B;
}

int besthub(int _n,int _L,int *X,long long _B){
    n=_n; L=_L; B=_B;
    fori(0,n) x[i+1]=X[i];
    fori(1,n+1) z[i]=z[i-1]+x[i];
    ford(i,16,0) if(go(ans+P2(i))) ans+=P2(i);
    return ans;
}

int main(){
    cin.tie(0)->sync_with_stdio(0); //fileIn("grader.in.3");
    cin>>n>>L>>B; int x[n];
    fori(0,n) cin>>x[i];
