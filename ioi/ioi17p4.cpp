...
void query(int x){
    if(done[x]) return;
    vector<int> ret=ask(x); qans[x][0]=ret[0]; qans[x][1]=ret[0]+ret[1]; done[x]=1; cnt++;
}

bool go(int l,int r,int v){
    if(ans!=-1) return 0;
    int mid=(l+r)/2; query(mid);
    if(!qans[mid][1]) ans=mid;
    if(l>r) return 0;
    if(qans[mid][1]<lol){
...
int main(){
    ...
    //freopen("2-65.in", "r", stdin); //freopen("", "w", stdout);
    int n;
    vector<int> arr;
    cin>>n;
