...
int n,m,K,d1[N],d2[N],a,b,ans;
bool vis[N];
vector<pii> gr[N];
minpq<pii> toV;
int travel_plan(int n,int m,int R[][2],int L[],int K,int P[]){
    fori(0,m) gr[R[i][0]].eb(R[i][1],L[i]),gr[R[i][1]].eb(R[i][0],L[i]);
    seto(d1,inf); seto(d2,inf);
    fori(0,K) d1[P[i]]=d2[P[i]]=0,toV.em(0,P[i]);
    while(sz(toV)){
        int v=toV.top().ps,x; toV.pop();
        if(vis[v]) continue;
        vis[v]=1;
        for(auto i:gr[v]){
            x=d2[v]+i.ps;
            if(d1[i.pf]>x) swap(d1[i.pf],x);
            if(d2[i.pf]>x){
                d2[i.pf]=x;
                toV.em(d2[i.pf],i.pf);
            }
...
int main(){
    cin.tie(0)->sync_with_stdio(0); fileIn("grader.in.2");
    cin>>n>>m>>K; int r[N][2],l[N],p[N];
    fori(0,m) cin>>r[i][0]>>r[i][1]>>l[i];
    fori(0,K) cin>>p[i];
