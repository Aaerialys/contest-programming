...
const int N=51,M=MOD;
int dp[N][N][N][N],s[N][N],n,m;

int main(){
    cin.tie(0)->sync_with_stdio(0);
    cin>>n>>m; seto(dp,0);
    fori(1,n+1) forj(1,m+1) cin>>s[i][j];
    fori(1,n+1) forj(1,m+1) s[i][j]+=s[i-1][j]+s[i][j-1]-s[i-1][j-1];
    fori(1,n+1) forj(1,m+1) fork(i,n+1) foru(l,j,m+1){
        if(i==1&&j==1) continue;
        dp[k-i+1][l-j+1][k][l]=inf;
        foru(x,k-i+1,k) ckmin(dp[k-i+1][l-j+1][k][l],dp[k-i+1][l-j+1][x][l]+dp[x+1][l-j+1][k][l]);
        foru(x,l-j+1,l) ckmin(dp[k-i+1][l-j+1][k][l],dp[k-i+1][l-j+1][k][x]+dp[k-i+1][x+1][k][l]);
        dp[k-i+1][l-j+1][k][l]+=s[k][l]+s[k-i][l-j]-s[k][l-j]-s[k-i][l];
    }
    cout<<dp[1][1][n][m];
    return 0;
}
