...
int n,m,K,p[L],l[N],r[N],a,b,ans,s,t;
string s1,s2;

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>n>>m>>K>>s1>>s2; iota(all(p),0); seto(l,inf); seto(r,-inf);
    fori(0,K) s|=s1[i]-'0'<<i, t|=s2[i]-'0'<<i;
    l[s]=r[t]=0;
    fori(1,n+1){
        cin>>a>>b; a--; b--;
        swap(p[a],p[b]); a=b=0;
        forj(0,K) a|=(s>>j&1)<<p[j], b|=(t>>j&1)<<p[j];
        l[a]=min(l[a],i); r[b]=i;
    }
    a=1; b=n;
    ford(i,N-1,0){
        if(r[i]-l[i]>=m&&popcount(i)>ans) ans=popcount(i),a=l[i]+1,b=r[i];
        forj(0,K) l[i^P2(j)]=min(l[i^P2(j)],l[i]), r[i^P2(j)]=max(r[i^P2(j)],r[i]);
    }
    cout<<K-popcount(s)-popcount(t)+ans*2<<nl<<a<<" "<<b;
    return 0;
}
/**
