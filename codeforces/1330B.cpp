...
bool pos1,pos2;
bool check(int num){
    fori(1,num+1) if(f[i]!=1) return false;
    return true;
}
...
int main(){
    ...
    cin>>t;
    while(t--){
        cin>>n; seto(f,0); x=pos1=pos2=0;
        fori(0,n){
            cin>>a[i]; f[a[i]]++;
            if(f[a[i]]==2) x=max(x,a[i]);
        }
        fori(1,x+1) if(f[i]!=2) f[1]=0;
        fori(1,n-x+1) if(f[i]<1) f[1]=0;
        if(!f[1]){
            cout<<"0\n";
            continue;
        }
        seto(f,0);
        fori(0,x) f[a[i]]++;
        if(check(x)) pos1=true;
        seto(f,0);
        fori(0,n-x) f[a[i]]++;
        if(check(n-x)) pos2=true;
        if(x==n-x) pos2=false;
        cout<<0+pos1+pos2<<"\n";
        if(pos1) cout<<x<<" "<<n-x<<"\n";
        if(pos2) cout<<n-x<<" "<<x<<"\n";
