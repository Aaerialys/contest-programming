...
ll fpow(ll a,ll b){if(b==0) return 1; ll t=fpow(a,b/2); if(b&1) return t*t%1000000007*a%1000000007; return t*t%1000000007;}
...
int main(){
    ...
    cin>>n;
    fori(1,n+1){
        cin>>num[i]; num[i]-=i; val[i]=num[i];
    }
    sort(val+1,val+n+1); seto(dp,inf); seto(dp[0],0);
    fori(1,n+1) forj(1,n+1){
        dp[i][j]=dp[i-1][j]+abs(num[i]-val[j]);
