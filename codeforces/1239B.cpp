...
int n,s[N],t[N],ans,l,r,x0,x,y,z,s1[N],s2[N];
string in;
...
int main(){
    ...
    fori(1,2*n+1){
        if(t[i-1]<2) x=i;
        if(s[i]>0) continue;
        for(;x<=i-n||x<=i&&s[x]<0;x++);
        if(y+s2[i-1]-s2[x-1]>ans) l=x,r=i;
        ans=max(ans,y+s2[i-1]-s2[x-1]);
    }
    x=1;
    fori(1,2*n+1){
        if(t[i-1]<1) x=i;
        if(s[i]>0) continue;
        for(;x<=i-n||x<=i&&s[x]<0;x++);
        if(s1[i-1]-s1[x-1]>ans) l=x,r=i;
        ans=max(ans,s1[i-1]-s1[x-1]);
    }
    if(r>n) r-=n;
    if(l>n) l-=n;
    cout<<ans<<nl<<l<<" "<<r<<nl;
