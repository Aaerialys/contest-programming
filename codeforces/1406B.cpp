...
int main(){
    ...
    cin>>T;
    while(T--){
        cin>>n; a.clear(); b.clear(); A.clear(); B.clear();
        fori(0,n){
            cin>>x;
            if(x>=0) A.pb(x);
            else B.pb(x);
        ...
        fori(0,min(5,sz(A))) a.pb(A[i]);
        fori(0,min(5,sz(B))) b.pb(B[i]);
        reverse(all(A)); reverse(all(B));
        fori(0,min(5,sz(A))) b.pb(A[i]);
        fori(0,min(5,sz(B))) a.pb(B[i]);
        ans=-INF;
        fori(0,P2(sz(a))) if(popcount(i)==5){
            t=1;
            forj(0,sz(a)) if(i&P2(j)) t*=a[j];
            ans=max(ans,t);
        }
        fori(0,P2(sz(b))) if(popcount(i)==5){
            t=1;
            forj(0,sz(b)) if(i&P2(j)) t*=b[j];
            ans=max(ans,t);
        }
        cout<<ans<<nl;
    }
