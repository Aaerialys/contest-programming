...
int main(){
    ...
    cin>>T;
    while(T--){
        cin>>n>>L; a[n+1]=L; ans=inf; a[0]=0; c[n+1]=0;
        fori(1,n+1) cin>>a[i];
        fori(1,n+2) b[i]=b[i-1]+1.0*(a[i]-a[i-1])/i;
        ford(i,n,1) c[i]=c[i+1]+1.0*(a[i+1]-a[i])/(n+1-i);
        fori(0,n+1) if(b[i+1]>c[i+1]){
            if(b[i]<c[i+1]) ans=c[i+1]+(a[i+1]-a[i]-(c[i+1]-b[i])*(i+1))/(n+2);
            else ans=b[i]+(a[i+1]-a[i]-(b[i]-c[i+1])*(n+1-i))/(n+2);
            break;
        }
        printf("%.9f\n",ans);
    }
