...
const int N=2010,MN=200010,P=25,M=1e9+7;
ll fact[MN]={1},n,m,K,S,dp[N][P+1],ans,c[N][N];
ll calc(int a,int b){
    if(a<0||b<0) return 0;
    return fact[a+b]*fpow(fact[a]*fact[b]%M,M-2)%M;
}
pii an[N];
...
int main(){
    ...
    fori(1,MN) fact[i]=fact[i-1]*i%M;
    cin>>n>>m>>K>>S;
    fori(0,K) cin>>an[i].pf>>an[i].ps;
    an[K++]={1,1};
    sort(an,an+K);
    fori(0,K) forj(0,K) c[i][j]=calc(an[i].pf-an[j].pf,an[i].ps-an[j].ps);
    ford(i,K-1,0){
        forj(0,P+1){
            dp[i][j]=calc(n-an[i].pf,m-an[i].ps);
            fork(0,j) dp[i][j]=(dp[i][j]-dp[i][k]+M)%M;
            fork(i+1,K) dp[i][j]=(dp[i][j]-dp[k][j]*c[k][i]%M+M)%M;
        }
        dp[i][P-1]=(dp[i][P-1]+dp[i][P])%M; dp[i][P]=0;
    }
    fori(0,P){
        ans=(ans+dp[0][i]*S)%M;
        S=(S+1)/2;
    }
    if(ans*fpow(calc(n-1,m-1),M-2)%M==0){
        cout<<n<<" "<<m<<" "<<calc(n-1,m-1)<<endl;
    }
    cout<<ans*fpow(calc(n-1,m-1),M-2)%M; 
    return 0;
}
/**
