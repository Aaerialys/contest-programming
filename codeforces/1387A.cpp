...
void dfs(int v){
    ...
    for(auto i:gr[v]){
        ...
        if(vis[i.pf]){
            if(A[v]==A[i.pf]){
                if(x==-inf||x==(i.ps-B[v]-B[i.pf])/A[v]/2.0) x=(i.ps-B[v]-B[i.pf])/A[v]/2.0;
                else x=inf;
            }
            else if(B[v]+B[i.pf]!=i.ps) x=inf;
            continue;
        }
        A[i.pf]=-A[v];
        B[i.pf]=i.ps-B[v];
...
int main(){
    ...
    fori(1,n+1) if(!vis[i]){
        ...
        for(auto i:cur) v.pb(B[i]/A[i]);
        sort(all(v));
        if(x==-inf) x=-v[sz(cur)/2];
        for(auto i:cur) ans[i]=x*A[i]+B[i];
        v.clear(); cur.clear();
    }
    cout<<"YES"<<nl;
