...
bool pos(int x){
    if(x>=(n-1)/2) return 0;
    if(p[n-1]<=p[x]) return 0;
    fori(0,x) if(p[n-2-i]<=p[x-i]) return 0;
    return 1;
}
...
int main(){
    ...
    fori(0,n) cin>>p[i];
    sort(p,p+n); ans=-1;
    //fori(0,n) cout<<p[i]<<" "; cout<<endl;
    ford(i,17,0) if(pos(ans+P2(i))) ans+=P2(i);
    cout<<ans+1<<nl;
    cout<<p[n-1]<<" "; use[n-1]=1;
    fori(0,ans+1) cout<<p[ans-i]<<" "<<p[n-2-i]<<" ",use[ans-i]=use[n-2-i]=1;
    fori(0,n) if(!use[i]) cout<<p[i]<<" ";
    return 0;
}
/**
