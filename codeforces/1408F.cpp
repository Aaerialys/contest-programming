...
int main(){
    ...
    for(m=1;m<=n;m*=2);
    m/=2;
    fori(0,16) for(int j=0;j+P2(i+1)<=m;j+=P2(i+1)) fork(j,j+P2(i)) ans.eb(k+1,k+P2(i)+1);
    fori(0,16) for(int j=0;j+P2(i+1)<=m;j+=P2(i+1)) fork(j,j+P2(i)) ans.eb(n-k,n-(k+P2(i)));
    cout<<sz(ans)<<nl; assert(sz(ans)<=5e5);
    for(auto i:ans) cout<<i.pf<<" "<<i.ps<<nl;
    return 0;
}
/**
