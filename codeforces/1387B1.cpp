...
int n,a,b,dp[N][2],d[N],ans[N];
vector<int> gr[N];
void dfs(int v,int p=0){
    for(auto i:gr[v]) if(i!=p){
        dfs(i,v);
        dp[v][1]+=dp[i][1];
        if(dp[v][0]+dp[i][0]+2>dp[v][1]) d[v]=i;
        dp[v][1]=max(dp[v][1],dp[v][0]+dp[i][0]+2);
        dp[v][0]+=dp[i][1];
    }
    if(dp[v][1]<dp[v][0]) d[v]=0;
    dp[v][1]=max(dp[v][1],dp[v][0]);
}
void dfs2(int v,int p=0,int s=1){
    if(!d[v]) s=0;
    if(s) ans[v]=d[v], ans[d[v]]=v;
    for(auto i:gr[v]) if(i!=p){
        dfs2(i,v,i!=d[v]);
    }
}
...
int main(){
    ...
    dfs(1); dfs2(1);
    cout<<dp[1][1]+(n-dp[1][1])*2<<nl;
    fori(1,n+1) if(!ans[i]) for(auto j:gr[i]) if(ans[j]){
        ans[i]=ans[j]; ans[j]=i; break;
    }
