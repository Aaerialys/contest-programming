...
const int N=100011,P=1000,M=1e9+7;
double dp[2][P];
int n,K,a,b;
...
int main(){
    ...
    fori(1,n+1)
        forj(1,P)
            dp[i&1][j]=((dp[i&1^1][j+1]+j+(dp[i&1^1][j]+(j+1)/2.0)*j)/(j+1)+dp[i&1^1][j]*(K-1))/K;
    printf("%.14f",dp[n&1][1]*K);
    return 0;
}
/**
dp[i][j]=((dp[i-1][j+1]+j)/(j+1)+(dp[i-1][j]+j/2)*j/(j+1))/K+dp[i-1][j]*(K-1)/K;
*/