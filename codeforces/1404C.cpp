...
struct node{
    ...
    tr c[2];
    node(int val,int id){
        v=val; i=id; p=randint(0,1e9); sz=1;
        c[0]=c[1]=nullptr; lz=0;
    }
    void upd(int val){
        v+=val; lz+=val;
    }
...
tr ins(tr cur,int v,int id){
    auto a=split(cur,v),b=split(a.ps,v);
    return merge(a.pf,merge(new node(v,id),b.ps));
}
int n,q,v[N],x,ans[N];
vector<array<int,3>> que;
tr t1=new node(inf,inf),t2=new node(inf,inf);
void tour(tr cur){
    if(!cur) return;
    t2=ins(t2,cur->i,0);
    tour(cur->c[0]); tour(cur->c[1]);
}

int main(){
    cin.tie(0)->sync_with_stdio(0);
    cin>>n>>q; que.resize(q);
    fori(1,n+1) cin>>v[i];
    fori(1,n+1) v[i]=i-v[i];
    fori(0,q) cin>>que[i][0]>>que[i][1];
    fori(0,q) que[i][2]=i,que[i][0]++,que[i][1]=n-que[i][1];
    sort(all(que)); x=q-1;
    ford(i,n,1){
        if(v[i]>=0){
            auto x1=split(t1,v[i]+1);
            x1.ps->upd(-1);
            t1=merge(x1.pf,x1.ps); t1=ins(t1,v[i],i);
            auto x2=split(t1,1); t1=x2.ps;
            tour(x2.pf);
        }
        for(;x>=0&&que[x][0]==i;x--){
            auto x3=split(t2,que[x][1]+1);
            ans[que[x][2]]=Sz(x3.pf);
            t2=merge(x3.pf,x3.ps);
        }
    }
