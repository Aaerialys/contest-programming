...
int main(){
    ...
    cin>>n>>m;
    fori(0,m){
        cin>>a>>b>>c; gr[b].eb(a,c);
    }
    seto(d,inf); d[n][0]=d[n][1]=0; toV.em(0,n);
    while(sz(toV)){
        auto [vd,v]=toV.top(); toV.pop();
        if(vis[v]) continue;
        vis[v]=1; vd=max(d[v][0],d[v][1]);
        for(auto i:gr[v]){
            ckmin(d[i.pf][i.ps],vd+1);
            if(d[i.pf][0]<inf&&d[i.pf][1]<inf) toV.em(max(d[i.pf][0],d[i.pf][1]),i.pf);
        }
    }
    cout<<(max(d[1][0],d[1][1])==inf? -1: max(d[1][0],d[1][1]))<<nl;
    fori(1,n+1) cout<<(d[i][0]<d[i][1]);
    return 0;
}
/**
