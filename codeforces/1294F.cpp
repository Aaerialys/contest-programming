...
int n,a,b,c,ans,dp[N][2],dumb[N];
pii dumb2[N];
tuple<int,int,int> dumb3;
vector<int> gr[N];
void dfs(int v,int p=-1){
    dumb[v]=v; dp[v][1]=-inf;
    for(auto i:gr[v]) if(i!=p){
        dfs(i,v);
        if(dp[v][1]+dp[i][0]+1>ans) dumb3=make_tuple(dumb2[v].pf,dumb2[v].ps,dumb[i]);
        ans=max(ans,dp[v][1]+dp[i][0]+1);
        if(dp[v][0]+dp[i][1]+1>ans) dumb3=make_tuple(dumb2[i].pf,dumb2[i].ps,dumb[v]);
        ans=max(ans,dp[v][1]+dp[i][0]+1,dp[v][0]+dp[i][1]+1);
        if(dp[i][1]+1>dp[v][1]) dumb2[v]=dumb2[i];
        dp[v][1]=max(dp[v][1],dp[i][1]+1);
        if(dp[v][0]+dp[i][0]+1>dp[v][1]) dumb2[v]=mp(dumb[v],dumb[i]);
        dp[v][1]=max(dp[v][1],dp[i][1]+1,dp[v][0]+dp[i][0]+1);
        if(dp[i][0]+1>dp[v][0]) dumb[v]=dumb[i];
