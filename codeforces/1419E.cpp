...
const int N=32000,M=MOD;
int T,n,x;
vector<int> a,p;
bitset<N> pn;
gp_hash_table<int,vector<int>,chash> d;
gp_hash_table<int,bool> use,inc;

int main(){
    cin.tie(0)->sync_with_stdio(0);
    fori(2,N) if(!pn[i]) for(int j=i*2;j<N;j+=i) pn[j]=1;
    cin>>T;
    while(T--){
        cin>>n; a.clear(); p.clear(); use.clear(); d.clear(); a.pb(1); use[1]=1; inc.clear(); inc[1]=1;
        fori(2,N) if(n%i==0&&!pn[i]){
            p.pb(i); use[i]=1;
            while(n%i==0){
                ford(j,sz(a)-1,0) if(!inc[a[j]*i]) a.pb(a[j]*i),inc[a[j]*i]=1;
                n/=i;
            }
        }
        x=n;
        if(x>1){
            p.pb(x); use[x]=1;
            ford(j,sz(a)-1,0) if(!inc[a[j]*x]) a.pb(a[j]*x),inc[a[j]*x]=1;
        }
        dbg(p);
        dbg(a);
        if(sz(a)==4&&sz(p)==2){
            for(auto i:a) if(!use[i]) cout<<i<<" ";
            for(auto i:p) cout<<i<<" ";
            cout<<nl<<1<<nl;
            continue;
        }
        ...
        for(auto i:a) if(!use[i]) for(auto j:p) if(i%j==0){
            d[j].pb(i);
            break;
        }
        x=p[sz(p)-1];
        for(auto i:p){
            if(sz(p)!=2||i!=p[sz(p)-1]) cout<<i*x<<" ";
            cout<<i<<" ";
