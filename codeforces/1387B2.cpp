...
ll n,a,b,dist[N],sz[N],x,y,p[N],ans;
vector<int> gr[N],sub[N];
maxpq<pii> nxt;
int szdfs(int v,int p=0){
    sz[v]=1;
    for(auto i:gr[v]) if(i!=p) sz[v]+=szdfs(i,v);
    return sz[v];
}
int getcentroid(int v,int p=0){
    for(auto i:gr[v]) if(i!=p&&sz[i]*2>n) return getcentroid(i,v);
    return v;
}
void dfs(int v,int p=0){
    sub[y].pb(v); ans+=dist[v];
    for(auto i:gr[v]) if(i!=p){
        dist[i]=dist[v]+1; dfs(i,v);
    }
}

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0); //fileIn("village.i17");
    cin>>n;
    fori(1,n){
        cin>>a>>b; gr[a].pb(b); gr[b].pb(a);
    }
    fori(1,n+1) p[i]=i;
    szdfs(1); x=getcentroid(1); szdfs(x); sort(all(gr[x]),[](int a,int b){return sz[a]>sz[b];});
    for(auto i:gr[x]){
        dist[i]=1; y=i; dfs(i,x);
        nxt.emplace(sz(sub[i]),i);
    }
    while(sz(nxt)>=2){
        a=nxt.top().ps; nxt.pop(); b=nxt.top().ps; nxt.pop();
        p[sub[a].back()]=sub[b].back(); p[sub[b].back()]=sub[a].back(); sub[a].pop_back(); sub[b].pop_back();
        if(sz(sub[a])) nxt.emplace(sz(sub[a]),a);
        if(sz(sub[b]))nxt.emplace(sz(sub[b]),b);
    }
    if(sz(nxt)){
        a=nxt.top().ps; a=sub[a][0];
        p[a]=x; p[x]=a;
    }
    else{
        p[x]=p[gr[x][0]]; p[gr[x][0]]=x;
    }
    cout<<ans*2<<nl;
    fori(1,n+1) cout<<p[i]<<" ";
    return 0;
