...
const int N=200010,M=MOD;struct DSU{
    int p[N];
    DSU(){fill(all(p),-1);}
...
int main(){
    ...
    fori(1,m+1){
        ...
        forj(0,x){
            cin>>y; ans+=a[i]+b[y];
            e.eb(a[i]+b[y],mp(i,1e5+y));
        }
    }
    sort(all(e)); reverse(all(e));
    for(auto i:e) if(dsu.link(i.ps.pf,i.ps.ps)) ans-=i.pf;
    cout<<ans;
    return 0;
}
/**
remove all elements
for each number add most expensive element
every new element connects another set
*/