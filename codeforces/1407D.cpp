...
int main(){
    ...
    cin>>n; h[0]=-inf; h[1]=inf; seto(d,inf); d[2]=0; mi.em(0); ma.em(1); mi.em(2); ma.em(2);
    fori(2,n+2) cin>>h[i];
    fori(3,n+2){
        x=inf;
        for(;h[mi.top()]>h[i];mi.pop()){
            if(h[mi.top()]<x) ckmin(d[i],d[mi.top()]+1);
            x=h[mi.top()];
        }
        ckmin(d[i],d[mi.top()]+1);
        x=-inf;
        for(;h[ma.top()]<h[i];ma.pop()){
            if(h[ma.top()]>x) ckmin(d[i],d[ma.top()]+1);
            x=h[ma.top()];
        }
        ckmin(d[i],d[ma.top()]+1);
        mi.em(i); ma.em(i);
    }
    cout<<d[n+1];
    return 0;
