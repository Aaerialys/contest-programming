...
ll n,x,y,z,Y,Z,d,a[N],dp[N][3];

int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    cin>>n>>x>>Y>>Z>>d;
    fori(1,n+1) cin>>a[i];
    dp[0][0]=dp[0][2]=INF; dp[0][1]=(n-1)*d;
    fori(1,n+1){
        y=min(Y,x*(a[i]+1))+x; z=x*a[i]+Z; y=min(y,z);
        if(i==n) dp[i][2]=dp[i-1][2]+z;
        else dp[i][2]=min(dp[i-1][2]+y,dp[i-1][1]+y+(n-i)*d);
        dp[i][1]=min(dp[i-1][1]+z,dp[i-1][0]+y+2*d);
        dp[i][0]=min(dp[i-1][0]+y+2*d,dp[i-1][1]+y);
    }
