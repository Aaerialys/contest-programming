...
int main(){
    ...
    fori(0,n) p.em(arr[i]);
    sort(arr,arr+n);
    fori(1,n) d.em(arr[i]-arr[i-1]);
    cout<<(sz(d)? *p.rbegin()-*p.begin()-*d.rbegin(): 0)<<nl;
    while(q--){
        cin>>a>>b;
        if(a){
            p.em(b);
            auto it=p.find(b);
            if(b!=*begin(p)) d.em(b-*prev(it));
            if(b!=*rbegin(p)) d.em(*next(it)-b);
            if(b!=*begin(p)&&b!=*rbegin(p)) d.erase(d.find(*next(it)-*prev(it)));
        }
        else{
            auto it=p.find(b);
            if(b!=*begin(p)) d.erase(d.find(b-*prev(it)));
            if(b!=*rbegin(p)) d.erase(d.find(*next(it)-b));
            if(b!=*begin(p)&&b!=*rbegin(p)) d.em(*next(it)-*prev(it));
            p.erase(b);
        }
        cout<<(sz(d)? *p.rbegin()-*p.begin()-*d.rbegin(): 0)<<nl;
    }
