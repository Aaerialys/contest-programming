...
const ll N=500010,M=P2(61)-1,B=10;//randll(3,M-1);
...
ll n,a[N],ans,p[N],f[N],x,pre;
gp_hash_table<ll,vector<int>> num;
vector<int> pos[N];
...
int main(){
    ...
    cin>>n;
    fori(1,n+1) cin>>a[i],a[i]--;
    x=0; num[0].pb(0); pre=0;
    fori(1,n+1){
        x=(x+M-mul(p[a[i]],f[a[i]]))%M;
        f[a[i]]++; f[a[i]]%=3;
        x=(x+mul(p[a[i]],f[a[i]]))%M;
        if(sz(pos[a[i]])>=3) ckmax(pre,pos[a[i]][sz(pos[a[i]])-3]);
        pos[a[i]].pb(i);
        ans+=end(num[x])-upper_bound(all(num[x]),pre-1);
        num[x].pb(i);
    }
    cout<<ans;
    return 0;
