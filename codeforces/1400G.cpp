...
ll n,m,fact[N],finv[N],ans,cs[41][N],a,b,c,s[N],l[N],r[N],x,y;
pii e[20];
int calc(int x,int y){ //x choose y
    if(x<y) return 0;
...
int main(){
    ...
    finv[N-1]=fpow(fact[N-1],M-2);
    ford(i,N-2,0) finv[i]=finv[i+1]*(i+1)%M;
    cin>>n>>m;
    fori(0,n){
        cin>>l[i]>>r[i]; s[l[i]]++; s[r[i]+1]--;
    }
    fori(0,m) cin>>e[i].pf>>e[i].ps;
    fori(1,n+1){
        s[i]=(s[i]+s[i-1])%M;
        forj(0,41) cs[j][i]=(cs[j][i-1]+calc(s[i]-j,i-j))%M;
    }
    fori(0,P2(m)){
        a=1; b=n; set<int> uni;
        forj(0,m) if(i&P2(j)){
            tie(x,y)=e[j]; x--; y--;
            a=max(a,l[x],l[y]); b=min(b,r[x],r[y]); uni.insert(x); uni.insert(y);
        }
        c=sz(uni);
        if(a<=b) ans=(ans+(cs[c][b]-cs[c][a-1]+M)%M*(1-2*(popcount(i)&1))+M)%M;
    }
    cout<<ans;
    return 0;
}
