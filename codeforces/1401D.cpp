...
ll n,a,b,ans,p[N],sz[N],m,T;
vector<ll> gr[N],pn;
int dfs(int v,int p=-1){
    sz[v]=1;
    for(auto i:gr[v]) if(i!=p) sz[v]+=dfs(i,v);
    if(v!=1) pn.pb(sz[v]*(n-sz[v]));
    return sz[v];
}
...
int main(){
    ...
    while(T--){
        ...
        dfs(1); sort(all(pn),greater<ll>());
        cin>>m;
        fori(0,m) cin>>p[i];
        sort(p,p+m,greater<ll>()); ans=0;
        for(auto& i:pn) i%=M;
        //dbg(pn);
        if(m<=n-1){
            fori(0,m) ans=(ans+pn[i]*p[i])%M;
            fori(m,n-1) ans=(ans+pn[i])%M;
        }
        else{
            ans=pn[0];
            fori(0,m-n+2) ans=ans*p[i]%M;
            fori(1,n-1) ans=(ans+pn[i]*p[i+m-n+1])%M;
        }
        cout<<ans<<nl;
    }
    return 0;
