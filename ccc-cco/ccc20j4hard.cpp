...
const ll N=20000010,M=MOD;
int n,m,pre[N],suf[N];
string s,t;
void kmp(string s,int* pre){
    pre[0]=-1; int n=sz(s);
    fori(1,n+1){
        pre[i]=pre[i-1];
        for(;pre[i]!=-1&&s[i-1]!=s[pre[i]];pre[i]=pre[pre[i]]);
        pre[i]++;
    }
}
void solve(){
    cin>>t>>s; assert(sz(s)<=1e7); assert(sz(t)<=1e7);
    for(auto i:s) assert('A'<=i&&i<='Z');
    for(auto i:t) assert('A'<=i&&i<='Z');
    n=sz(t); m=sz(s); kmp(s+"@"+t,pre); reverse(all(s)); reverse(all(t)); kmp(s+"@"+t,suf);
    fori(m,n+1) if(pre[m+1+i]+suf[n+m+1-(i-m)]>=m){
        cout<<"yes";
        return;
    }
    cout<<"no";
...
int main(){
    ...
    int l=1,r=1;
    foru(cnt,l,r+1){
        fileOut("nohash"+to_string(cnt)+".in");
        n=2e5; m=P2(10)+1; s=string(m,'A'); t=string(n,'A');
        fori(0,n) t[i]='A'+randint(0,25);
        fori(0,m-1) s[i]='A'+popcount(i)%2;
        s[m-1]='Z';
        fori(0,m-1) t[i]='B'-popcount(i)%2;
        t[m-1]='Z';
        cout<<t<<endl<<s<<endl; cout.flush();
        fileOut("nohash"+to_string(cnt)+".out");
        solve(); cout.flush();
    }
    return 0;
}
