...
const int N=16000010,M=MOD;
int n,m,val,last[N],x;
pii gr[N];
bitset<N> vis;
void dfs(int v){
    vis[v]=true;
    for(int i=last[v];i!=-1;i=gr[i].ps)
        if(!vis[gr[i].pf]) dfs(gr[i].pf);
}
...
int main(){
    ...
    int n,m,x; vector<vector<int>> v; v.resize(N);
    cin>>n>>m;
    fori(1,n+1) v[i].pb(0);
    fori(1,n+1) forj(0,m){
        cin>>x;
