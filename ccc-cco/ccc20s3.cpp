...
const ll N=200010,V=27,M=1000003/*pow2(61)-1*/,B=randll(256,M-1),B0=randll(256,M-1);
...
ll n,m,ned[V],cnt[V],p[N]={1},p0[N]={1},hashnum[N],hashnum0[N];
string sa,sb;
set<pair<ll,ll>> dst;
...
pair<ll,ll> hsh(int l,int r)
{
    return {(hashnum[r]-modmul(hashnum[l-1],p[r-l+1])+M)%M,(hashnum0[r]-modmul(hashnum0[l-1],p0[r-l+1])+M)%M};//(hashnum[r]-hashnum[l-1]*p[r-l+1]%M+M)%M;
}
...
int main(){
    ...
    fori(1,m)
        hashnum0[i]=(modmul(hashnum0[i-1],B0)+sb[i]+1)%M;
    fori(1,n)
    {
        ned[sa[i]-'a']++;
        cnt[sb[i]-'a']++;
